...
-- | Half a game-minute of retained progress: an ordinary, exactly
--   representable mid-minute value for the totality examples, which must
--   hand back the remainder they were given as well as the clock.
...
spec = do
    describe "advanceWorldClock" $ do
        ...
        it "does not advance when the scale is zero (paused), and keeps \
           \the sub-minute progress it was carrying" $ do
            -- #2471: a paused tick runs the same arithmetic at scale 0,
            -- so the remainder must come back BIT-IDENTICAL rather than
