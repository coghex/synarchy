...
spec = describe "Generated language names" $ do
    ...
    describe "profile generation (requirements 1, 2, 12)" $ do
        ...
        -- Version 5 stands in for "never existed". This test used to
        -- name versions 2, 3, and then 4, which #1094, #1095 and #1096
        -- made real — a rejection test must always point at a version
        -- outside the supported set, never at the next one about to be
        -- implemented.
        it "rejects an unsupported version descriptively rather than falling back to version 1" $ do
            let r = generateProfile (GeneratorVersion 5) (LangSeed 7)
            r `shouldBe` Left (UnsupportedGeneratorVersion 5)
            case r of
        ...
        it "version 4 is the current generator, with versions 1, 2 and 3 \
           \still constructible beside it" $ do
            currentGeneratorVersion `shouldBe` GeneratorVersion 4
    ...
    describe "bound-form selection (#1096 requirements 2, 3, 5)" $ do
        ...
        it "stores only nonempty strictly-shorter prefixes that retain a \
           \visible letter (requirement 3)" $ do
            let visible t = T.any (\c → isAsciiUpper c ∨ isAsciiLower c) t
                offenders =
                    [ (profSeed p, c, free, b)
