...
resetScene env start = do
    writeIORef (gameTimeRef env) 0
    writeIORef (unitManagerRef env) emptyUnitManager
        { umDefs = HM.singleton "red_squirrel"
                       (minimalDef "red_squirrel" "Red Squirrel")
        , umInstances = HM.singleton squirrelUid (mkSquirrel (statsWith start)) }
    _ ← Q.flushQueue (unitQueue env)
    pure ()

-- * Live readers

storedPressure ∷ EngineEnv → IO (Maybe Float)
storedPressure env = do
    um ← readIORef (unitManagerRef env)
    pure (HM.lookup squirrelUid (umInstances um)
            ⌦ HM.lookup "sleep_pressure" ∘ uiStats)

-- | The stored value, or a failure naming the pool that went missing.
readPressure ∷ EngineEnv → IO Float
readPressure env = storedPressure env ⌦ \case
    Just v  → pure v
    Nothing → do
        expectationFailure "the fixture's sleep_pressure entry is gone"
        pure 0
...
tickN ls params activity pose dt n = do
    r ← evalDebug ls $ T.concat
        [ "local p = ", params, "; "
        , "for _ = 1, ", tshow n, " do "
        , "_G.__tick.tickResource(1, 'red_squirrel', 'sleep_pressure', "
