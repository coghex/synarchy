...
--
--   Runs on the shared stdlib-only harness
--   ('Test.Headless.Preview.LuaHarness'), which drives the REAL shipped
--   @preview_zoom@ / @unit_animation_view@ / @building_asset_view@ /
--   @preview_manager@ Lua with only @engine@\/@UI@ and the two modules
--   NOT under test stubbed. See that module for why a reimplementation
--   would prove nothing here.
module Test.Headless.Preview.Zoom (spec) where

import UPrelude
import Test.Hspec
import Test.Headless.Preview.LuaHarness
    ( harness, lns, runsOk )
...
spec = do
  ...
  describe "buildings viewer" $ do
    it "zooms the panel sprite and preserves the multiplier across an \
      ...
      , "-- Selection speaks in row IDENTITIES since #2492; a YAML-less"
      , "-- payload like this one has only filesystem rows."
      , "assetBrowserStub.selectEntry(1, 'filesystem:default.png')"
      , "pm.update(0.016)"
      , "local after = pm.dump()"
