    ...
    around withHeadlessEngine $ do
        ...
        describe "scrolling" $ do
            it "scrolls by code points and shows whole characters" $ \env → do
                ls ← shellBackend env
                -- 100px per code point against the shell's 1012px input
                -- field: exactly ten characters fit, so a fifteen-character
                -- line must scroll five of them away.
                --
                -- #1959 is why that field is 1012px and not the 1100px this
                -- case measured against before: the console now fits its box
                -- to the framebuffer, and 1280px cannot hold the preferred
                -- 1368px-wide box, so the center narrows to 1112 and the
                -- input line takes its 100px inset out of that. The stub
                -- framebuffer below is unchanged — the width it implies is
                -- what moved.
                eval ls
                    ...
                    `shouldReturn`
                        "15|15|5|10|🙂é界🙂é界🙂é界🙂|3|3|10|é界🙂é界🙂é界🙂é"
        ...
        describe "ASCII behaviour is unchanged" $ do
            it "edits, navigates, completes, and scrolls exactly as before" $ \env → do
                ...
                eval ls
                    ...
                    `shouldReturn` "abXc|3|ab|2|0|2|qqr|3|15|5|aaaaaaaaaa"
