...
gatingSpec = describe "Blood.Trail.consumeTrailMarks — distance/cadence gating" $ do
    ...
    it "pops nothing when distance hasn't been covered, even with ample time and volume" $ do
        let ts0 = TrailState { tsPendingVolume = 5.0, tsDistSinceMark = 0, tsLastMarkAt = 0 }
            (ts', marks) = consumeTrailMarks defaultTrailThresholds 0.1 100.0 ts0
        marks `shouldBe` []
    ...
    it "pops nothing when cadence hasn't elapsed, even with ample distance and volume" $ do
        let ts0 = TrailState { tsPendingVolume = 5.0, tsDistSinceMark = 0, tsLastMarkAt = 100.0 }
            (ts', marks) = consumeTrailMarks defaultTrailThresholds 50.0 100.01 ts0
        marks `shouldBe` []
    ...
    it "pops exactly one mark once BOTH gates are cleared" $ do
        let tp  = defaultTrailThresholds
            ...
            (_, marks) = consumeTrailMarks tp (ttMinDistance tp) (ttMinCadence tp) ts0
    ...
    it "never pops a mark with no real blood behind it, even when both gates clear" $ do
        ...
        let tp  = defaultTrailThresholds
            ...
            (ts', marks) = consumeTrailMarks tp (ttMinDistance tp) (ttMinCadence tp) ts0
        marks `shouldBe` []
...
partitionInvarianceSpec =
  describe "Blood.Trail.consumeTrailMarks — timestep-partition invariance" $ do
    ...
    it "an evenly-divisible path splits into the SAME total marks/volume regardless of step count" $
...
-- | Shared partition-invariance check (issue #882 spec addition): the
--   same continuous path (@distGates@/@cadenceGates@ multiples of the
--   thresholds) and external-loss budget (@totalVol@), split into a
--   single big update vs @steps@ small ones, must converge to the SAME
--   emitted mark count and total volume — within a documented
--   tolerance, since chunking can shift exactly which step a boundary-
--   crossing mark pops on by at most one (Float summation: e.g.
--   @0.05*10@ landing a hair below @0.5@). Volume conservation itself
--   (popped + still-pending = everything ever added) is checked
--   EXACTLY for both the single big call and the many-small-steps run
--   — nothing may be invented or dropped regardless of how the same
--   journey is chunked.
...
checkPartitionInvariance distGates cadenceGates totalVol steps = do
    let tp = defaultTrailThresholds
        ...
        (bigFinal, bigMarks) = consumeTrailMarks tp totalDist totalTime
        ...
        goSteps _ ts acc 0 = (ts, acc)
        goSteps clock ts acc k =
            let ts1 = ts { tsPendingVolume = tsPendingVolume ts + stepVol }
                clock' = clock + stepTime
                (ts2, popped) = consumeTrailMarks tp stepDist clock' ts1
            in goSteps clock' ts2 (acc ++ popped) (k - 1 ∷ Int)
        (smallFinal, smallMarks) = goSteps (0 ∷ Double) emptyTrailState [] steps
