...
drawFrameOffscreen ∷ EngineM ε σ ()
...
renderSceneFrame ∷ Word32 → Word32 → FrameResources → Device
                 → EngineM ε σ (CommandBuffer, Maybe PendingCapture)
...
updateUniformBufferForFrame ∷ Word32 → Camera2D → EngineM ε σ ()
...
--   only pacing.
submitFrameOffscreen ∷ CommandBuffer → FrameResources → DevQueues → EngineM ε σ ()
