...
--   RGBA8 is not a page this format wrote. Only then does the native
--   decoder run, inside an exception boundary with its output fully
--   forced, because JuicyPixels inflates through zlib, which can throw
--   on a corrupt deflate stream instead of returning 'Left'.
...
checkPagePngHeader what png = do
    ...
    unless (word32At 8 ≡ 13 ∧ BS.take 4 (BS.drop 12 png) ≡ "IHDR") $
        Left $ MapArtifactPngHeader what "the first chunk is not a 13-byte IHDR"
    let width  = word32At 16
        height = word32At 20
  ...
  where
    word32At i = foldl' (\acc k → acc `shiftL` 8 ⌄ fromIntegral (BS.index png (i + k)))
                        (0 ∷ Word32) [0 .. 3]
...
--   native decoder under an exception boundary, then the decoded length
--   against the page plan.
