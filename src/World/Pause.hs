...
--   already zero.
--
--   Every engine-side pause writer goes through here. It deliberately
--   does not take the player-intent lock
--   ('Engine.Core.Capability.WorldSim.withPlayerIntent'): an
--   engine-imposed pause is not player intent, and one caller
--   ('Engine.Scripting.Lua.API.Save.acceptSaveRequest') already holds
--   that lock when it calls in.
