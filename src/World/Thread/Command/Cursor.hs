...
-- * Till designation tool (#333)
--
--   Mirrors the mine/construct designation tools' anchor→rectangle
--   commit: per-z-level like mining (a farmed field is flat ground),
--   but the commit ALSO filters out tiles that can't be tilled — fluid
--   on top (no floor to till), an existing flora instance (can't till
--   under a standing plant/tree), or already-tilled ground (idempotent
--   re-sweep). The till AI (scripts/unit_ai.lua) is the consumer;
--   completion goes through the generic world.setVegAt primitive, not a
--   till-specific one (mirrors how chop completion calls
--   world.harvestFlora then chop.cancelDesignation).
