...
| `unit_resources` | `scripts/unit_resources.lua` | global (per-id caches) | Reset to default | — | reset hook (`registerResetHook`, not a save component). It clears TWO per-unit caches, both deliberately never persisted and both cleared on every load, including a load carrying no data for this module at all. `alerts.resetOnLoad()` clears the alert-debounce cache, so a reused unit id (post `umNextId` rewind) can't inherit stale suppression state. `unit_resource_carry.resetOnLoad()` (#2633) clears the sub-binary32 remainders `scripts/unit_resource_carry.lua` holds for the last resource write it made: each is meaningless without the stored `uiStats` value it is a remainder OF, and a load replaces those wholesale. Nothing is owed across the boundary — a remainder is at most half an ulp, never a deferred change, because the tick writes every time. SESSION LIFECYCLE (#1610): Exit to Menu runs NONE of this — no `applyAll`, so no reset hook — and it replaces the entity managers and can rewind `umNextId` exactly as a load does, so `unitResources.init` registers the SAME clear with `scripts/lib/session_teardown.lua` under the same id (the two registries are independent namespaces). `unitResources.update` also holds off on `sessionTeardown.isTornDown()` through the `UnitClearAll` drain window, as `unitAi.update` and `buildingSpawn.update` do: the destroyed session's units are still reported by `unit.getAllIds` in that window, and ticking them would re-enter both caches the teardown just cleared | `Test.Headless.Unit.ResourceTickCarry` (`--match "resource tick sub-threshold carry"`, §6) |
