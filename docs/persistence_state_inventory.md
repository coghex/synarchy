...
| `wsUnitFloorRef` | Exclude | — | #2476: the exclusive lower bound on the `UnitId`s belonging to THIS incarnation of the page — a unit below it was admitted for an incarnation this one replaced. Written once, inside the page/entity lifecycle transition that registers the state and before it is reachable through `wmWorlds`, from the same `umNextId` reading that transition gives its queued `UnitClearPage` as a cutoff, so the two agree by construction. Read by the verbs that turn a unit into DURABLE page-scoped state — `power.placeNode`'s supplier check and `unit.createTransferOrder`'s store resolution — which refuse a unit below it, because a page id is a reusable NAME and teardown is queue-ordered: without it a doomed unit could commit a building, a power node or a transfer order onto the replacement that outlives the unit itself. Unrestorable for the same reason `wsChunkResidencyRef`'s epoch is — it names a boundary between two generations of a page id WITHIN one process, and a saved value would name a generation that no longer exists. `UnitId 0` (`emptyWorldState`'s value, below every allocated id) is the correct state for a page that was never replaced and for every page a LOAD publishes, since a save's own unit ids must not be measured against a departed session's allocator | hspec `--match "Page incarnation entity teardown"` |
