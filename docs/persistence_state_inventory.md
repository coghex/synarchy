...
| `TutorialRegistry.trTree` | `src/Tutorial/Types.hs:139` | global | Rebuild | `src/Engine/Asset/YamlTutorials.hs` | whole-tree validation at load (#957): unique ids, required presentation/evaluator fields, known kinds, resolvable references, valid kind/relationship combinations, exactly one root, no orphans, no cycles, one parent per objective. A file failing any check publishes nothing, so this slot is either the fully validated tree or explicitly `Nothing` — never partial. | hspec `--match "Tutorial definitions"` |
