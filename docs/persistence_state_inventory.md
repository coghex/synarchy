...
| `WorldPagesDTO` | `world-pages` (v3) | Persist exactly | the world page — page-set authority: identity (name, gloss, and the optional `LanguageProvenanceDTO` language provenance #1092 added), gen params (incl. the placed-location instance table and its page-local allocator, #911), dates/clocks, map mode, per-page camera. v2 (pre-#1092) decodes through `migrateWorldPagesV2` and v1 (pre-#911, per-chunk location flags) through `migrateWorldPagesV1` + `resolveLegacyLocations`; both — and the frozen v90 session (`World.Save.Compat.SessionV90`) — carry name/gloss across exactly and produce ABSENT provenance, never inferred (#915's precedent). |
