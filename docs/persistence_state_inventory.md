...
| `wmSessionTeardown` | Exclude | none — never written to or read from a save | set by `World.Thread.Command.Basic.handleWorldDestroyAllCommand` in the same atomic update that empties the page set and cleared by `Unit.Thread.endSessionEpoch` as the last act of the Exit-to-Menu boundary, so it can only be true while that cross-thread transition is genuinely outstanding; `World.Load.Publish.publishStagedSession` installs it false, which covers the one path that DISCARDS the queue markers whose drain would otherwise clear it | hspec `--match "Exit to Menu session epoch"` |
