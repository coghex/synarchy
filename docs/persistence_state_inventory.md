...
| Audio preview selection, categories, control handles, reload revision and one-shot autoplay intent | `scripts/ui/preview_audio.lua` | preview pane | Exclude | authored audio catalogs and explicit CLI file | reopen/resize preserves current selection; teardown clears all pane state, reload replaces metadata without playback | `Audio.PreviewUI`; `tools/preview_probe.py --only audio` |
