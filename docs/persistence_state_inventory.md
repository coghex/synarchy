...
| `audioTransport` | global | Exclude | — | transient semantic queues, listener, epoch and process-scoped volume settings; no SaveData field | `Audio.Transport`, `Audio.Runtime` hspec |
| `audioStatusRef` | global | Exclude | — | pointer-free diagnostic snapshot including copied preview metadata/revision; starts disabled and is republished by worker | `Audio.Thread` hspec |
...
| `zoomAtlasDataRef` | global | Exclude | — | pending GPU upload payloads, one per image still waiting for the render thread. A QUEUE since #2485 rather than a single slot: an accepted solidification republishes its page's atlas at runtime, so two pages committing between render frames would otherwise have left one image silently overwritten. The two rebuild paths differ because what they rebuild differs: a world INIT rebuilds one page, so it supersedes only that page's own pending image and leaves the rest queued, while a LOAD PUBLISH replaces the session and so clears the queue whole — unconditionally, even when the incoming session carries no atlas, since everything still pending names a page about to stop existing | hspec `--match "solidification"`, `--match "generated page identity"` |
...
| `materialRegistryRef` | global | Rebuild | `data/materials/*.yaml` plus this process's live `engine.loadMaterialYaml` registrations | rebuilt from shipped YAML on every world init and on load (`Engine.Asset.YamlMaterials.loadPopulatedMaterialRegistry`), with the LIVE registry merged over that base so a custom id registered from a file outside `data/materials` survives — live registrations win on an id collision (#763 for load, #2278 for world init). Never serialized: saves reference materials by numeric id and a load rejects an id this build never registered. | hspec `--match "live material registrations survive world.init"` |
...
| `pageLifecycleLock` | global | Exclude | — | #2476: runtime-only mutex (`MVar ()`) linearising every entity ADMISSION (`unit.spawn`, `building.spawn`, `power.placeNode` — the only three sites that allocate a `UnitId` or `BuildingId`) against every page LIFECYCLE transition (single-page `world.destroy`, and either init path replacing a registered page id). The transition captures `umNextId`/`bmNextId` under it and enqueues page-scoped clears carrying those readings as exclusive cutoffs, so an id allocated before the transition is provably retired and one allocated after provably survives. Process-lifetime and value-free — it is a critical section, not state — so there is nothing a save could record and nothing a load could restore. | `test-headless/Test/Headless/World/PageIncarnation.hs` |
...
| `wmTeardownsPending` | Exclude | none — never written to or read from a save | counts Exit-to-Menu boundaries STARTED but not completed: incremented by `World.Thread.Command.Basic.handleWorldDestroyAllCommand` in the same atomic update that empties the page set, decremented (clamped at zero) by `Unit.Thread.endSessionEpoch` as the last act of each boundary, so it is non-zero exactly while some cross-thread teardown is genuinely outstanding; `World.Load.Publish.publishStagedSession` installs it at zero, which covers the one path that DISCARDS the queue markers whose drain would otherwise decrement it | hspec `--match "Exit to Menu session epoch"` |
| `wmSessionEpoch` | Exclude | none — never written to or read from a save | #2512: which SESSION the page set belongs to, bumped by the two transitions that replace one wholesale — `World.Thread.Command.Basic.handleWorldDestroyAllCommand` and `World.Load.Publish.publishStagedSession` — each in the same atomic step that installs the new page set, so the epoch and the state it names can never be observed apart. It is what lets a queued `WorldRecordPortableKnowledge` be REFUSED when the session it was measured against is gone: FIFO orders that command against a teardown, but a teardown queued AHEAD of an observation would otherwise clear the map and then have the departed session's memory inserted back into it. Distinct from `wmSelectionGen`, which answers whether the ACTIVE PAGE moved and deliberately does not move when a teardown finds nothing visible. Not persisted: it names this process's session sequence, and a restored session takes a fresh value from the publish that installed it | hspec `--match "Portable container knowledge"` |
| `wmPortableKnowledge` | Persist exactly | the optional session component `portable-knowledge` (§10) | the SESSION-scoped owner of #2512's portable-container memory, keyed by `Item.Types.iiInstanceId`. Written ONLY by the world thread — `World.Thread.Command.Basic.handleWorldRecordPortableKnowledgeCommand`, which merges the `PortableObservation` a Lua verb measured and queued, so a caller-thread write can never land a departed session's memory in the one that replaced it. Captured by `World.Thread.Command.Save.WriteWorld` into `SessionGlobals.sgPortableKnowledge`; installed by `World.Load.Publish.publishStagedSession` as part of the REPLACEMENT manager, so a load replaces it wholesale and an absent payload clears it rather than leaving the outgoing session's memories standing. A record whose item instance is absent from the incoming session's own live item enumeration — read off each staged page's ground ref AFTER every reconciliation pass, so a refunded item counts — is SCRUBBED with a diagnostic during staging (`World.Load.Stage`), never a load failure — the same tolerated-dangling judgement `container-knowledge` gets for a demolished building. Emptied by `World.Thread.Command.Basic.handleWorldDestroyAllCommand` in the same atomic update that clears the page set: an instance id is only unique within a session. Remembered instance ids are historical observations and are excluded from `allItemInstanceIds`, the allocator bound, the duplicate check and live `item_instance` resolution; their def names are validated by `World.Save.Types.missingPortableItemDefReferences` | hspec `--match "Portable container knowledge"` |
...
| `wsTexturesRef` | Exclude | — | runtime GPU handles, including the eight exact-fluid masks; initial and late binding rebuilds them | `World.Render.FluidLevels` binding/cache tests |
| `wsGenParamsRef` | Persist exactly | — | `wpsGenParams`; deliberately meaningful seed data (contract §1) | `tools/multiworld_save_probe.py` |
| `wsTimeRef` | Persist exactly | — | `wpsTimeHour`/`wpsTimeMinute`, plus #2471's `wpsTimeRemainder` (`world-pages` v11): this ref holds a `PreciseWorldTime` — the whole minutes AND the sub-minute calendar progress they cannot store — as ONE value, because the world thread writes it while other threads read it and a two-ref publication could pair a new minute with a stale remainder. The remainder is repaired to zero — never refused — at `World.Load.Stage` when the stored value is not a fraction of a game-minute (NaN, an infinity, negative, or one minute or more); `validatePages` deliberately leaves it unjudged so every such payload reaches that one repair. A pre-v11 save migrates to zero, which is the value it actually recorded: the old clock floored the fraction away every tick. | `tools/save_pause_probe.py`, `tools/persistence_contract_probe.py` (three fresh-process save→load→save cycles compared through the real codec, all while paused), `Test.Headless.World.SubMinuteClock` |
...
| `wsZoomLiveRef` | Rebuild | this page's own zoom cache + the generated chunks it was built from | #2485: the RGBA atlas pixels THIS page holds, plus the colour palette they were generated with and the atlas layout, kept so an accepted terrain edit can regenerate one chunk's 32x32 block and republish the image the zoom renderer samples. Session-local presentation data, derived entirely from the page's own zoom cache. EVERY page that has a zoom map retains one: `World.Thread.Command.Init` writes it on a fresh world, and `World.Load.Stage` writes it for every staged page, not only the one whose image is handed to the GPU at load (#1670 still governs WHO receives an upload — a page never renders through an atlas another page's cache produced — while this is what each page keeps for itself; a non-owner's first live refresh is what publishes its own). `Nothing` only for a page with no zoom map at all: an arena, or a page whose atlas the device refused, which drops its zoom cache with it so no map can silently stop tracking the world. A load re-establishes it from the pixels it stages rather than from anything a save could carry. Nothing derives from it; it is never read as gameplay state | hspec `--match "solidification"`, `--match "World identity"`, `tools/fluid_reaction_visual_probe.py` |
...
| `wsStructureDestructionsRef` | Exclude (session-transient) | — | the transient, render-only teardown presentations of cleared structure pieces (#2491): captured by `WorldClearStructure` in the same handler that records the authoritative `WeClearStructure` edit and deletes the resident overlay entry, pruned by the world tick against the game clock, and owning no gameplay authority — every structure query walks `lcStructures` and `wsStructureStageRef` and never this. Never in `WorldPageSnapshot`; every constructed page (boot, arena init, load staging, `structure.clearAll`) starts with none, so a save taken mid-playback restores neither the piece nor its effect | hspec `--match "structure destruction presentation lifecycle"` |
...
| `wsGeneratedIdRef` | Persist exactly | — | `wpsGeneratedId`, authoritatively (#2021): the opaque `GeneratedWorldId` naming the generated FOUNDATION this page descends from, distinct from `wsIdentityRef` (display text), the routing `WorldPageId`, the save-slot name, and the file path. Non-optional live: `emptyWorldState` mints one, so every page — `WorldInit`, arena, or load-staged — has exactly one from the instant it exists. A save older than `world-pages` v9 carries none, and load staging simply keeps the freshly minted id rather than deriving one from the legacy save; loading the same legacy save twice therefore yields two different ids (D-21), which is accepted. Never derived from the seed, the display name, the page id or a slot name — two worlds generated from identical parameters get different ids (requirement 3, D-17) | hspec `--match "generated world identity"` |
...
| `smGeneratedWorldIds` | Persist exactly | EVERY page's `wpsGeneratedId` at save time (not just the active page's) | #2021: a duplicate-free, ascending COPY of the authoritative `world-pages` values, carried here so a `listSaves`-depth read — and later reference-aware library cleanup — can obtain the save's complete set of generated foundations without decoding any gameplay component, exactly as `smWorldName` is. Both copies are written from one value (`World.Save.Snapshot.Adapter.snapshotSaveMetadata`); a save whose two copies disagree is refused for full load by `World.Save.Component.metadataErrors`, never silently resolved in favour of either. `"metadata"` v1 and v2 payloads have no such field and migrate EMPTY (those formats predate the identity) via `World.Save.Compat.MetadataV1`/`MetadataV2` | hspec `--match "generated world identity"` |
...
| `wpsTimeRemainder` | Persist exactly | — | domain-checked at load, not at decode (#2471): `World.Load.Stage` accepts a finite value in `[0, 1)` and loads anything else as `0` with a warning naming the page and the stored value. Losing under a minute of clock is never worth refusing the rest of a save. | `tools/persistence_contract_probe.py` (see §12), `Test.Headless.World.SubMinuteClock` |
...
| `wpsEdits` | Persist exactly | `wpsGenParams` (edits replay onto regenerated terrain) | replayed edits must reproduce the pre-save surface exactly for every edited tile. #2243: the species a planted-flora edit names is the authored YAML `name` (`World.Flora.Reference.FloraRef`), never the runtime `FloraId` — the catalog that numbers species is rebuilt at boot and never saved, so an ordinal only means anything to the build that wrote it. `World.Thread.Command.Save.WriteWorld` names every live handle at capture and refuses the save if its own catalog cannot; `World.Load.Stage.stageSession` resolves each name back to this build's handle, after `missingFloraReferences` has refused any load naming a species this build lacks. A pre-#2243 payload's ordinal is carried forward verbatim by the migration and resolved against the LOADING build's catalog, which may name a different species — D-2 in `docs/flora_species_identity_design.md`, documented rather than guarded | `tools/multiworld_save_probe.py`, `tools/world_check.py` (determinism) |
...
| `wpsGroundItems` | Persist exactly | — | instance ids must be below `sdNextItemInstanceId` — validated at SAVE time over the whole session (`World.Save.Snapshot`'s `itemAllocatorErrors`), which is what the restore relies on. #2336: an entry whose stored position is not finite is the one thing NOT restored — `Item.Ground.sanitizeGroundItems` drops it during staging (`World.Load.Stage`, one warning per entry naming the page and the page-local ground-item id), and the SANITIZED map is what the staged page installs, what the transfer-order integrity diagnostic resolves against, and what `knownEntitiesFromSaveData` reports — so the Lua reference-edge check and the post-load reconciliation context (`ssReconcile`) both describe the restored session, and a durable AI reference to a dropped entry is cleared rather than left live. `gisNextId` and the global item cursor are untouched, so a dropped id is retired rather than reissued. Non-blocking by design, on #1087's container-knowledge terms: a phantom the live spawn boundaries now refuse outright is damage from an older build, not corruption, so it is diagnosed and dropped, never a load failure — and an order or significant obligation naming it stays retained and dangling | `tools/item_instance_probe.py`, hspec `--match "non-finite spawn geometry"` |
...
| `wpsUnits` | Persist exactly | `umDefs` must already have every referenced unit def (contract §4: missing def fails load); the infection catalogue must be registered before staging reconciles acquired immunity | referenced def names must resolve. #2305: `uisImmunities` is the one field NOT restored verbatim — `World.Save.Types.fromUnitSnapshot` drops every acquired-immunity entry whose key does not resolve against the registered infection catalogue (one immutable view per `stageSession` attempt, so a save's pages cannot be reconciled against different definition sets), and `World.Load.Stage` logs the count and the distinct unresolved ids per page. Non-blocking by design, on #1087's container-knowledge terms: an immunity key is a memory of content the unit already survived, so it is diagnosed and dropped, never a load failure. A wound's `woundInfectionType` is the opposite half of the same reference and stays a hard rejection through `missingInfectionReferences`. Resolving keys keep their exact levels, and the session saves back without the orphan | `tools/multiworld_save_probe.py`, hspec `--match "acquired-immunity scrub"` |
...
| `wpsCropPlots` | Persist exactly | referenced tile coordinates must be within the page | #2243: the species a planted-flora edit names is the authored YAML `name` (`World.Flora.Reference.FloraRef`), never the runtime `FloraId` — the catalog that numbers species is rebuilt at boot and never saved, so an ordinal only means anything to the build that wrote it. `World.Thread.Command.Save.WriteWorld` names every live handle at capture and refuses the save if its own catalog cannot; `World.Load.Stage.stageSession` resolves each name back to this build's handle, after `missingFloraReferences` has refused any load naming a species this build lacks. A pre-#2243 payload's ordinal is carried forward verbatim by the migration and resolved against the LOADING build's catalog, which may name a different species — D-2 in `docs/flora_species_identity_design.md`, documented rather than guarded, and the same holds for a plot's `cpSpecies` | `tools/crop_probe.py` |
| `wpsPlantDesignations` | Persist exactly | referenced tile coordinates must be within the page | its `ptCrop` names the species by authored YAML `name`, on the same terms `wpsEdits` above spells out (#2243) — and it is validated: the pre-#2243 `missingFloraReferences` never walked this field at all, so a designation naming a species nothing could plant used to load clean. Same claimant caveat as `wpsMineDesignations`. #1858 adds one post-restore reconciliation, the `wpsContainerKnowledge` pattern applied to terrain rather than buildings: tilled soil is a CONTINUOUS validity requirement, so a restored record whose tile is RESIDENT and no longer tilled soil is SCRUBBED — at `World.Load.Stage` for the terrain that load reconstructs, and again as each further chunk publishes (`World.Plant.Validate`). Never a load failure, and never inferred from silence: a record whose chunk is not resident is UNKNOWN and is retained until its terrain resolves | `tools/plant_probe.py`, `Test.Headless.World.CropPlant` (`--match "crop plant invalidation"`) |
| `wpsIdentity` | Persist exactly | — | none beyond type-correctness | `tools/multiworld_save_probe.py` |
| `wpsGeneratedId` | Persist exactly | — | #2021: the page's opaque `GeneratedWorldId`, carried across the in-memory load bridge so `World.Load.Stage` can restore it onto `wsGeneratedIdRef`. `Nothing` only for a save whose `world-pages` payload predates v9; staging then keeps the staged page's own freshly minted id and never writes it back to the source file | hspec `--match "generated world identity"` |
...
| `sdPortableKnowledge` | Persist exactly | the optional session component `portable-knowledge` (§10) | #2512's session-scoped portable-container memory, crossing the in-memory load bridge UNDUPLICATED — it is genuinely global, so unlike the camera and the id allocators there is no legacy per-page slot to fabricate a copy into. Restores onto `WorldManager.wmPortableKnowledge` (§3) through `World.Load.Stage`'s scrub and `World.Load.Publish`'s replacement manager | hspec `--match "Portable container knowledge"` |
...
| `umInstances` | Persist exactly | via `UnitSnapshot`/`wpsUnits`, needs `umDefs` resolved first | missing species def fails load (contract §4). Since #2307 the restore additionally RECONCILES each unit's `uiEquipment` against the equipment class its current `UnitDef` declares (`Equipment.Reconcile.reconcileUnitEquipment`, called from `fromUnitSnapshot` and therefore on every staged load): an entry whose slot id the class no longer declares — including every entry on a unit whose def declares no class, or whose class no longer resolves in `EquipmentClassManager` — and an entry whose item `idKind` no longer matches that slot's `esKind` are both invalid, and the EXACT `ItemInstance` moves to the unit's loose inventory rather than being dropped, recreated or re-rolled. Non-blocking on #1087's footing: one `logInfo` per migrated entry naming the unit, the affected slot, the item def, the instance id and which of the two causes applied, never a load failure. The item DEFINITION such an entry names stays a whole-load rejection (`missingItemDefReferences`), so an entry whose def does not resolve is deliberately left in its slot rather than migrated. Ordering is deterministic: the saved loose inventory keeps its own order and position and the migrated entries are appended in slot-id order, matching `unitHeldItems`. Carried weight is unchanged — `unitGetCarryingWeightFn` weighs inventory and equipment identically — and the item becomes visible and removable through the ordinary inventory UI, which enumerates the current class's slots and could never render it before. | `tools/multiworld_save_probe.py`; hspec `--match "saved equipment slot reconciliation"` |
...
| `bmReservations` | Exclude (session-transient) | — | footprint claims for buildings admitted but not yet committed (#2326): taken by `Building.Reservation.reserveFootprint` in the same transition that allocates the `BuildingId`, consumed by the transition that inserts the instance, and released by every path that drops one instead (world gone, unknown def, stale page binding, `BuildingClearAll`). Never in `BuildingSnapshot`; every constructed manager (boot, load replacement, `BuildingClearAll`) starts with none, so a spawn admitted in the outgoing session has no claim on the incoming one's tiles and is refused at commit | hspec `--match "Building footprint exclusivity"` |
...
|---|---|---|---|---|---|---|
| Audio catalog, preview file configuration and disposable catalog/revision, decoded PCM, native device/ring/voices, loop bindings, cooldowns, health windows, bounded diagnostic cache and publication counters | `src/Engine/Audio/Thread.hs`, `src/Engine/Audio/Runtime.hs`, `src/Engine/Audio/Health.hs`, `cbits/audio/` | one audio worker | Exclude | authored audio catalog and local player-volume config | assets rebuild at startup; session replacement clears voices/loops/cooldowns/ring and stale commands; callback joins before freeing; process-lifetime diagnostic history ends on worker teardown | `Audio.Native`, `Audio.Runtime`, `Audio.Thread`, `Audio.Health`; `tools/test_audio_native.py` |
...
| `SimWorldState`'s `swsSolidEvents` (solidification-event history) | `src/Sim/State/Types.hs` | per-page, nested within that page's `SimWorldState` entry | Exclude | — | **Transient simulation OUTPUT, not scratch that can be recomputed.** Unlike the blanket "Rebuild" above, this collection is NOT reconstructible from the final fluid grid: it records unlike-fluid contacts that have already happened (#2481), and the grid they happened in has moved on. It is initialized EMPTY by `emptySimWorldState` — at boot and at load reconstruction alike — is drained by the world-side consumer — since #2485 every emitted delivery takes the whole collection, grouped into coherent `ReactionResult`s stamped with each participating chunk's live-edit generation — and is removed with its simulated page (`SimDropWorld`; `SimDeactivateWorld` keeps the world entry, so events survive hide/show like the chunks do, and an inactive world emits nothing and therefore drains nothing). A pending reaction that a save interrupts is therefore lost by design, exactly as an in-flight tick's partial results are. No simulation record has a `Serialize` instance and the save format does not change | hspec `--match "unlike-fluid reaction"` (the event-accumulation group), `--match "solidification"` (the drain) |
...
| `Lua.Thread`'s debug-console `debugQueue` (`TQueue DebugCommand`, `dcCommand`/`dcResponse`/`dcState`) | `src/Engine/Scripting/Lua/DebugServer/Types.hs`, `src/Engine/Scripting/Lua/Thread.hs` | global (one debug TCP server for the whole engine) | Exclude | — | queued-but-unprocessed debug-console commands (their response `MVar`s and their `dcState` lifecycle cells alike) at a snapshot boundary are cancelled, not persisted or replayed — an open debug-shell session has no gameplay meaning (contract §1/§5: exclude open debug-shell contents). Since #2282 that cancellation is the shared `cancelDebugCommand` transition: a command cancelled at the boundary becomes `DebugCommandCancelled <reply>`, cannot subsequently be claimed, and therefore cannot execute against the replacement session however long it sits on the queue. At the load boundary the waiting client is answered with that rejection text rather than left without a response; at engine teardown the console is stopped before the drain runs (#2170), so a socket client's connection simply closes and the cancellation — not the reply — is the part that matters | hspec `--match "LuaSaveLoaded stale debug-command cancellation"`, `--match "debug-console command cancellation"` |

### Lua runtime call telemetry (#2483)
...
| `unit_resources` | `scripts/unit_resources.lua` | global (per-id caches) | Reset to default | — | reset hook (`registerResetHook`, not a save component). It clears THREE per-unit caches, all deliberately never persisted and all cleared on every load, including a load carrying no data for this module at all. `alerts.resetOnLoad()` clears the alert-debounce cache, so a reused unit id (post `umNextId` rewind) can't inherit stale suppression state. `unit_resource_carry.resetOnLoad()` (#2633) clears the sub-binary32 remainders `scripts/unit_resource_carry.lua` holds for the last resource write it made: each is meaningless without the stored `uiStats` value it is a remainder OF, and a load replaces those wholesale. Nothing is owed across the boundary — a remainder is at most half an ulp, never a deferred change, because the tick writes every time. `unit_resource_regrowth.resetOnLoad()` (#2637) clears the sub-binary32 fat/lean/body residues and the implicit organ baseline in `scripts/unit_resource_regrowth.lua`. A residue is at most half an ulp of its stat, never a deferred change and never unapplied clamp debt. The baseline is `body_mass - fat_mass - lean_mass`, captured when regrowth takes authorship of that composition and kept across that unit's own ticks so later quantization cannot redefine it; it is not serialized. After a reset the next regrowth tick reconstructs it from the stored masses, so the loss is bounded by those half-ulp residues. An external write of `fat_mass`, `lean_mass` or `body_mass` drops that authorship, which keeps the intervening composition instead of replaying a stale total. SESSION LIFECYCLE (#1610): Exit to Menu runs NONE of this — no `applyAll`, so no reset hook — and it replaces the entity managers and can rewind `umNextId` exactly as a load does, so `unitResources.init` registers the SAME clear with `scripts/lib/session_teardown.lua` under the same id (the two registries are independent namespaces). `unitResources.update` also holds off on `sessionTeardown.isTornDown()` through the `UnitClearAll` drain window, as `unitAi.update` and `buildingSpawn.update` do: the destroyed session's units are still reported by `unit.getAllIds` in that window, and ticking them would re-enter the caches the teardown just cleared | `Test.Headless.Unit.ResourceTickCarry` (`--match "resource tick sub-threshold carry"`, §6), `Test.Headless.Unit.RegrowthPrecision` (`--match "surplus regrowth retains sub-Float32 increments"`) |
| `unit_ai_claims` | `scripts/unit_ai_claims.lua` (registered from `scripts/unit_ai_save.lua`; owners live in `unit_ai_dig`/`_chop`/`_construct`/`_farm`/`_repair` and `unit_ai_encounter`) | global, session-wide transient coordination | Reset to default | — | reset hook (`registerResetHook`, not a save component, and deliberately not under the persistent component's `unit_ai` id). `claimsLib.resetAll()` empties all ELEVEN registries in place on every successful load: five coordinate work claims, `repairClaims`, `repairPriority`, and #916's four same-tick encounter overlays (`localEpisodeActive`, `localEpisodeAggression`, `localEpisodeDisengaged`, `localParticipation`). None lives in `aiState`. The work tables must not survive rewound entity ids/clocks; the encounter overlays must not override the newly loaded encounter's persisted episode/occupant state when page and instance ids are reused. Content-data caches remain deliberately excluded. Session teardown reaches the same registered family through the existing unit-AI lifecycle. | `Test.Headless.Lua.UnitAiLoadReset` (`--match "unit AI load reset"`), `tools/construction_probe.py` |
...
| Audio preview selection, categories, control handles, reload revision and one-shot autoplay intent | `scripts/ui/preview_audio.lua` | preview pane | Exclude | authored audio catalogs and explicit CLI file | reopen/resize preserves current selection; teardown clears all pane state; any observed preview-revision advance replaces metadata and rows without playback, whichever preview caller requested the reload | `Audio.PreviewUI`; `tools/preview_probe.py --only audio` |
| Audio Settings current/pending/saved values and slider/label handles | `scripts/settings/data.lua`, `scripts/settings/audio_tab.lua` | Settings screen | Exclude | player-volume config, outside world saves | rebuild preserves pending values; semantic entry reads current/saved values, Back restores the saved baseline; handles are destroyed with the screen | `Audio.Settings` hspec; `tools/audio_manual_probe.py --offscreen-check` |
| `debugOverlay` module state (per-mode `entries`/`listVisible`/`buttonId`, each mode's `armedField` selection, `modeOrder`, hit-test layout state) | `scripts/debug.lua:43-73` | global (Lua module singleton) | Exclude | — | transient debug-overlay UI state (open panel, armed spawn/edit selection, mode list); Grab alone has the reset hook classified below; explicitly required Exclude by contract §5 ("debug overlays"). Already reset to fresh per-mode defaults at `require`/reload time (`scripts/debug.lua:69-73`), independent of save/load. | none yet |

| Debug Grab armed flag, captured page/generation/gid/instance, fractional pointer offset and press claims (`debug_grab` reset hook) | `scripts/debug.lua`, `scripts/debug/grab.lua` | global (Lua gesture, page-qualified capture) | Reset to default | — | Never serialized. Load apply reset hook and saveLoaded reconciliation disarm/cancel; Exit to Menu uses session teardown. Hide, view transitions, Escape, right-click, resize, mode changes and identity/page loss end capture without rollback. Press ownership lasts until release to suppress other subscribers; it cannot revive capture. | `Debug Grab gesture (#2489)`; rendered capture and generated-world save/load evidence |
...
| `EquipmentClassManager.ecmDefs` | `src/Equipment/Types.hs:43` | global | Rebuild | `src/Engine/Asset/YamlEquipment.hs` | none beyond type-correctness for the registry itself, but since #2307 it is the SCHEMA a saved equipment map is reconciled against at staging: `World.Load.Stage.stageSession` reads it (with `ItemManager`) exactly once per load attempt, never per page, so one save's pages cannot be reconciled against different slot schemas — see `umInstances` in §5 for what the reconciliation does. | `tools/repair_item_probe.py`; hspec `--match "saved equipment slot reconciliation"` |
...
| `LootProfileRegistry.lprDefs` | `src/LootProfile/Types.hs:65` | global | Rebuild | `src/Engine/Asset/YamlLootProfiles.hs` | whole-file validation at load (#2499): a document is refused outright for a missing, empty, non-string or REPEATED `id`, empty or absent `entries`, a `chance` outside the inclusive `[0, 1]` or non-finite in the stored 32-bit `Float`, a `quantity_factor` that is not a positive whole number, a `quantity_multiplier` with `min < 1` or `max < min`, and — after a successful decode — an `item` that is not in the live item registry (D-20; items load before profiles). A refused file registers NOTHING, so a previously registered profile of the same id survives a bad replacement intact. | hspec `--match "Loot profiles"`, `tools/content_registry_probe.py` |
...
| `FloraCatalog` (`fcSpecies`, `fcWorldGen`, `fcNextId`) | `src/World/Flora/Types.hs:244` | global | Rebuild | `src/Engine/Asset/YamlFlora.hs` | this type derives `Serialize`/`Generic` unlike its sibling content registries, but nothing in `SaveData` embeds it; species are referenced from world state by numeric id at RUNTIME and by authored YAML `name` in a SAVE (#2243) — the catalog is the only thing that relates the two, which is why it is read at exactly two boundaries and nowhere else (`World.Flora.Reference`). #763 (round 5), extended by #2243: every species reference in the edit log, crop plots (`cpSpecies`) and plant designations (`ptCrop`) is resolved against this catalog before publication; a load referencing a species this build does not register is rejected, naming the species. A pre-#2243 payload's ordinal is resolved against the loading build's catalog and may name a different species (D-2, documented rather than guarded). | `tools/flora_growth_probe.py`, `tools/transactional_load_probe.py` |
...
|---|---|---|---|
| `SaveMetadata` (metadata component) | `metadata` (v3) | Persist exactly | Envelope — listing metadata, readable without decoding gameplay. v3 (#2021) appends `smGeneratedWorldIds`, the save's complete duplicate-free inventory of per-page `GeneratedWorldId`s, so a listing-depth read can name every generated foundation the save references. v2's shape is frozen as `World.Save.Compat.MetadataV2.SaveMetadataV2` and v1's as `MetadataV1.SaveMetadataV1`; both migrate with that inventory EMPTY, because those formats predate the identity — a value is never inferred from the seed, the world name, or anything else in the payload. |
...
| `WorldPagesDTO` | `world-pages` (v13) | Persist exactly | the world page — page-set authority: identity, generation parameters, dates/clocks, map mode, and per-page camera. Generation parameters contain #911's placed-location instance table and page-local allocator, #1102's `GeoFeatureId`-keyed river-name table, and the terrain-generation inputs. A location stores its resolved bounds, display name/gloss, optional #1104 etymology source, lifecycle, content-spawn flag, and — since #916 — its optional encounter: the one-time count roll, roster-complete/death-only/cleared flags, activation/current-episode/once-per-episode feedback state, and every occupant's same-page unit id, distinct home tile, and engaged/returning state. v8 freezes the pre-encounter v7 shape as `PageCoreDTOv7`/`WorldGenParamsDTOv6`/`LocationInstancesDTOv4`/`LocationInstanceDTOv4`; `migrateWorldPagesV7` preserves the materialized locations and adds no encounter. The v1 reconstruction path likewise strips any encounter supplied by today's definitions. Earlier migrations remain version-specific: v6 drops #1230's obsolete discovery margin; v5 predates etymology sources; v4 predates river names; v3 predates location glosses; v2 predates language provenance; v1 predates location instances and resolves its per-chunk flags through `resolveLegacyLocations`. Historical absent fields stay absent rather than being inferred from current content or regenerated data. v9 (#2021) appends each page's opaque `GeneratedWorldId` — the AUTHORITATIVE copy, of which `metadata` v3 carries a duplicate for cheap scanning — and freezes the pre-#2021 shape as `PageCoreDTOv8`; `migrateWorldPagesV8` (and every earlier migration, through `blankPageSnapshot`) leaves the id ABSENT rather than deriving one, and `World.Load.Stage` mints a fresh id per page during transactional staging instead. `validatePages` refuses a payload whose OWN version carries the field but omits it (`wpIdsFromPayload`), and refuses two pages naming one foundation. Since #917 it also stores its GUARANTEED SIGNIFICANT-item obligations (per slot: the authored item def name, the spawned item's physical `iiInstanceId` once bound, and its own `taken` latch) plus the one instance-level clearance-notice latch that #917 generalized OUT of the encounter, so a location authoring significant items and no encounter has one too. `leCleared` is now encounter completion ALONE; the cleared LIFECYCLE is the conjunction of the conditions a location actually authors, and a location authoring neither never clears. v10 freezes the pre-#917 v9 shape as `PageCoreDTOv9`/`WorldGenParamsDTOv7`/`LocationInstancesDTOv5`/`LocationInstanceDTOv5`/`LocationEncounterDTOv1`; `migrateWorldPagesV9` preserves every stored value (#2021's generated-world id included, so `wpIdsFromPayload` stays true), lifts the encounter's clearance-notice flag onto the instance so a deferred notice is neither replayed nor lost, and adds NO significant obligations — reading them off today's YAML would owe a materialized world an item it never spawned. #2021's own v8 freeze had its gen-params field repointed onto that same `WorldGenParamsDTOv7`, leaving its bytes unchanged. v8 freezes the pre-encounter v7 shape as `PageCoreDTOv7`/`WorldGenParamsDTOv6`/`LocationInstancesDTOv4`/`LocationInstanceDTOv4`; `migrateWorldPagesV7` preserves the materialized locations and adds no encounter. The v1 reconstruction path likewise strips any encounter AND any significant obligation supplied by today's definitions. Earlier migrations remain version-specific: v6 drops #1230's obsolete discovery margin; v5 predates etymology sources; v4 predates river names; v3 predates location glosses; v2 predates language provenance; v1 predates location instances and resolves its per-chunk flags through `resolveLegacyLocations`. Historical absent fields stay absent rather than being inferred from current content or regenerated data. v11 (#2471) appends each page's sub-minute clock remainder — the game-minutes already elapsed that the whole-minute `pcTimeHour`/`pcTimeMinute` pair cannot hold — and freezes the pre-#2471 shape as `PageCoreDTOv10`; `migrateWorldPagesV10` (and every earlier migration, through `blankPageSnapshot`) loads it as ZERO, which is the value those saves actually recorded rather than an absence being filled in: the pre-#2471 clock floored the fraction away on every tick. The wire field is an unjudged `Double` and `validatePages` deliberately does not check it, so an out-of-domain stored value reaches `World.Load.Stage`'s single repair (zero, with a warning naming the page) instead of refusing the whole save over under a minute of clock. v12 (#2505, epic #1231 PLC-14) appends each placed location's PENDING CONTAINER SHELLS: per slot, the authored container item definition, the loot-profile id its cargo will one day be realized from, the spawned shell's physical `iiInstanceId` once bound, and its own `realized` latch. Slots are numbered over the definition's `kind: container` entries alone, in a separate address space from the significant obligations, and — unlike those — they confer NO clearance condition (`conditionsOf` never reads them, design D-18): a location whose only content is a crate still authors no clearance and still never clears. The pre-#2505 shape is frozen as `PageCoreDTOv11`/`WorldGenParamsDTOv8`/`LocationInstancesDTOv6`/`LocationInstanceDTOv6`; `migrateWorldPagesV11` preserves every stored value (#2471's sub-minute remainder and #2021's generated-world id included, so `wpIdsFromPayload` stays true) and adds NO container slots — reading them off today's YAML would owe a materialized world a crate it never spawned against an instance whose `contents_spawned` guarantees nothing will ever spawn it. #2471's own v10 freeze had its gen-params field repointed onto that same `WorldGenParamsDTOv8`, leaving its bytes unchanged, exactly as #917 did to #2021's v8 freeze. The v1 reconstruction path likewise adds no container slot. `validatePages` additionally runs `locationContainerSlotErrors`, which rejects a slot below 1, a bound id of 0, a duplicated slot number, a `realized` slot naming no shell, and CROSS-FAMILY duplicate ownership — one physical id claimed by a container slot and a significant obligation alike, which one global allocator makes impossible to be two real items. #2533 v13 stores `gtWorldRivers.wrByChunk.rcePerTileSurfZ` as signed eighth-z units with `minBound` absent slots. The frozen `WorldPagesDTOv12` / `WorldGenParamsDTOv9` preserves the outgoing wire shape. v13 also persists `wgpExactRiverBeds`: new worlds apply minimal river-only bed repair after historical terrain smoothing; all pre-v13 migrations set it false so old beds retain their generation policy through resaving. The `GeoTimeline` wire shape is unchanged; every v1–v12 migration scales real heights exactly once, including the legacy-session V1 path. River masks, widths, identities, stored carve and unrelated timeline fields remain unchanged; new saves preserve fractional values exactly. Width-claim and breakthrough-path scratch exists only during generation and is not persisted. Gates: `WorldGen.ExactRiver`, tracked `z5-exact-generated-rivers`, save migrations and real-restart compatibility probes. |
| `WorldEditsDTO` | `world-edits` (v4) | Persist exactly | world edit layer — per-page terrain + structure edit log. v4 (#2520, epic #2514 DFL-2) changes no field and no constructor tag: `WeSetFluidSnapshotD`'s four fields are exactly the ones v3 wrote. What the bump buys is the MEANING of the last one — the column's fluid surface is now a SIGNED fixed-point ABSOLUTE height in eighths of a z-level (`World.Fluid.Exact`, eight units per z), not a whole z, so the partial top level the simulation really holds survives the round trip instead of being rounded up to a full one. The precision promise is exact: what the sim wrote is what a load reconstructs, unit for unit, and a save can never turn a one-unit cell into a whole level. The pre-#2520 shape is frozen as `WorldEditDTOv3`, and `migrateWorldEditDTOv3` scales a historical `surfaceZ` to exactly `surfaceZ * 8` — a FULL top level — inventing no fractional remainder those bytes never recorded. v1 and v2 payloads reach that hop through `migrateWorldEditsV1`/`migrateWorldEditsV2`, so every accepted older version is scaled exactly ONCE and a v4 payload is not scaled again. Tracked evidence: the `z3-exact-fluid-units` baseline is a real saved session whose snapshots carry partial top levels under several fluid types, Ocean included. Its canonical summary pins the plane in three layers: the page totals; a PER-TYPE histogram of how many cells sit at each top fill level 1..8; and an `fnv1a64` `digest` over every snapshot in canonical coordinate order, each carrying its coordinate, type and exact surface. Each closes a hole the one above leaves open — a type swap survives the totals, and a compensating `+8`/`-8` pair or a permutation of two cells' surfaces survives the histogram too, because a histogram is a multiset. `samples` names the coordinate-first witness of each (type, level) so a digest mismatch reads as real cells with real exact surfaces. What is persisted is the exact SURFACE, not volume-over-terrain (terrain is regenerated, not stored); the volume-over-terrain identity is proved at a known terrain by the hspec round trip. The whole-z Lua and dump views cannot see a remainder at all. |
...
| `PortableKnowledgeDTO` | `portable-knowledge` (v1, **OPTIONAL**) | Persist exactly | `Item.Knowledge` — what the player remembers about each PORTABLE container, keyed by the item's own `iiInstanceId` (#2512, epic #1231 PLC-7): per crate, an optional weight observation (the recursive `itemTotalWeight` plus the game-time it was weighed) and an optional contents observation (full instance copies plus the game-time they were seen), each independently present. The THIRD optional component in this static Haskell registry, and the FIRST that is SESSION-scoped rather than page-scoped: a crate is carried between pages and owners, so its memory is keyed by the crate and written once for the whole session rather than as a per-page slice — which is also why it declares no dependency on `world-pages`. Every tracked compatibility baseline predates it, and an ABSENT payload means "no crate has ever been hefted or opened" — true rather than invented, because before this slice there was nowhere to record either — restoring `Item.Knowledge.emptyPortableKnowledge`, never known-empty and never back-filled from what the live item holds now. A PRESENT payload that is malformed, or encoded at a version this reader does not accept, still fails the load. `csValidate` refuses a remembered weight or either observation timestamp that is not a finite, non-negative number, and deliberately does NOT re-derive the weight from the remembered contents (it was measured THEN, and it measures the whole crate rather than its contents list) nor check that a remembered id still names a live instance — that is the tolerated load-boundary scrub in `World.Load.Stage`. Its remembered `ItemInstanceDTO`s are historical observations, excluded from `allItemInstanceIds`, the allocator bound, the duplicate-live-id check and live `item_instance` reference resolution; their def names remain ordinary content references, validated by `World.Save.Types.missingPortableItemDefReferences` BEFORE the scrub can discard the records they belong to. |
...
| `leodUnitId` | `LocationEncounterOccupantDTO` (`world-pages`) | Same page as the placed location encounter | Persist as identity/reference | `World.Save.Integrity.sessionIntegrityErrors` (wrong-page hard-fails); absent-everywhere is tolerated and reported by `sessionIntegrityWarnings`, with roster membership retained so it cannot falsely clear | none of its own; the occupant shape is unchanged since #916, but it rides `LocationEncounterDTOv1` on the frozen world-pages v9 boundary and the live shape on v10. v7 and older decode with no encounter at all |
| `lsidInstanceId` | `LocationSignificantItemDTO` (`world-pages`) | Same page as the placed location that owes the item — and, while the obligation is UNTAKEN, on that page's GROUND specifically | Persist as identity/reference | `World.Save.Integrity.significantProvenanceErrors` hard-fails an untaken obligation whose item resolves on another page or in an inventory/storage (it cannot be held without having been picked up, which is the one thing that latches `taken`), and hard-fails one physical id owed by two obligations; `significantDanglingWarnings` reports absent-everywhere and tolerates it, leaving the obligation untaken so a missing item can never be mistaken for a satisfied condition. It also hard-fails ANY bound id at or above the session's `snapNextItemId` cursor, taken or not: the monotonic allocator could never have minted it, and a taken entry is exactly where such a forgery would otherwise hide, since 'gone' is indistinguishable from 'never existed' to a resolution check. Once `lsidTaken` is true no OTHER rule applies — post-pickup loss, consumption and destruction are explicitly allowed. `Location.Instance.locationSignificantItemErrors` additionally rejects a slot below 1 (unbindable, so the content spawn would orphan an item on every load), a bound id of 0 (never minted, and the one value the taken-skipping provenance rules cannot catch), a duplicated slot, an obligation marked taken that names no item (the one shape the provenance rules above cannot see, having no id to resolve), and same-page duplicate ownership, at component decode. An untaken obligation's item must also BE the definition the slot names, checked at the binding boundary and again at save. The clearance predicate itself also refuses to count a taken-but-unbound obligation, so such a payload cannot clear a location even if it reached a live table. Absent until the content spawn binds one, which is what keeps an unspawned obligation incomplete — and while it is absent the sibling `lsidItemDefName` is validated against the live item registry by `World.Save.Types.missingSignificantItemReferences`, so a save owing an item this build no longer defines is refused rather than published into a location that could never spawn it. A BOUND obligation is exempt: nothing re-spawns a filled slot. | none (world-pages v10; v9 and older decode with NO obligations — `fromLocationInstanceDTOv5` never infers them from today's YAML) |
| `lcsdInstanceId` | `LocationContainerSlotDTO` (`world-pages`) | Same page as the placed location that owns the slot — and, while the slot is UNREALIZED, an OUTER GROUND item on that page specifically | Persist as identity/reference | `World.Save.Integrity.containerProvenanceErrors` hard-fails a pending shell that resolves on another page, in an inventory, in a building's store, or nested inside another container ("outer" is `PageEntities.peGroundItems`, never the flattened `peItems`: an id that exists only inside a ground container is not pickable as its own ground item, so PLC-15 could never realize it), one that is a DIFFERENT item definition from the one the slot names, and any bound id at or above the session's `snapNextItemId` cursor. It also hard-fails one physical id claimed by more than one location slot ACROSS both families — container and significant alike — since item ids come from one global allocator; that walk reports every group containing at least one container claim, while a group of significant claims alone stays `significantProvenanceErrors`' to report, so the two partition the cases rather than double-reporting the overlap. `containerDanglingWarnings` reports a bound-but-absent shell and TOLERATES it — `item.removeGround` deletes a ground item outright, so a scripted removal of a pending shell reaches exactly that state; the slot then stays pending for ever (`contents_spawned` is already set, so nothing re-spawns it) and no cargo is ever realized from it. Once `lcsdRealized` is true NO rule applies at all: design D-3 discards the profile and the source at realization, and the shell becomes an ordinary item that may be carried, stored, nested or destroyed. `Location.Instance.locationContainerSlotErrors` additionally rejects a slot below 1, a bound id of 0 (never minted), a duplicated slot number, and a `realized` slot naming no shell, at component decode. The sibling `lcsdProfile` is validated against the live loot-profile registry by `World.Save.Types.missingContainerProfileReferences` for every UNREALIZED slot, BOUND or unbound — the deliberate divergence from `missingSignificantItemReferences`, which is restricted to unspawned obligations: a bound obligation's def name is a historical record of an item already made, while a bound shell's profile is still a future draw, and design D-23 makes a realization that cannot complete refuse the pickup, so a shell whose profile is gone is a crate nobody could ever lift. | none (world-pages v12; v11 and older decode with NO container slots — `fromLocationInstanceDTOv6` never infers them from today's YAML) |
| `codItems`' `itdInstanceId`s (#2512) | `ContentsObservationDTO` (`portable-knowledge`) | none — NOT a reference | Exclude | nothing: this is the one persisted item id in the save that is deliberately NOT a durable reference, so no scope applies and no resolution is attempted. A remembered crate's contents are COPIES of what was inside it when the player looked, and staleness REQUIRES those ids to stay meaningful after the real items have moved, changed or ceased to exist — which is why they are absent from `World.Save.Snapshot.allItemInstanceIds`, from the allocator bound, from the duplicate-live-id check, and from `Engine.Scripting.Lua.API.Save.Integrity.knownEntitiesFromSaveData`'s `keItemInstances`. The consequence is load-bearing and asserted directly: a Lua `item_instance` edge naming a remembered-only id does NOT resolve — `World.Save.Integrity.luaReferenceErrors` reports it as a tolerated dangling reference exactly as it would for an id that never existed — so remembering a crate can never make a stale live reference look live. Their DEF NAMES are ordinary content references and ARE validated, by `World.Save.Types.missingPortableItemDefReferences` | none: a frozen v1 shape with no earlier version to migrate from |
...
| `world-pages` | `PageSnapshot`'s identity/gen-params/dates/clocks/map-mode/per-page-camera fields, plus #2021's `pgsGeneratedId` and #2471's `pgsTimeRemainder` (added at v11: the sub-minute calendar progress beside the whole-minute clock; `migrateWorldPagesV10` loads every pre-v11 payload with none, which is what those saves recorded). Gen params include `wgpLocationInstances` (allocator, stored geometry/name/provenance/lifecycle/content state, and #916's optional encounter roll, exact roster, occupant homes, activation/current-episode feedback, and guard-policy state, #917's significant-item obligations plus the instance-level clearance-notice latch, and — since v12 — #2505's PENDING CONTAINER SLOTS, each naming its container definition, its loot profile, its bound shell once spawned and its `realized` latch; `migrateWorldPagesV11` adds none to a v11 payload) and #1102's page-local river-name table. #2021's generated-world id and its `metadata` v3 counterpart are pinned by hspec `--match "generated world identity"`, which also covers the four ways the two copies can disagree and the fresh-id-at-staging rule for a pre-v9 save. | `tools/persistence_contract_probe.py`, `Test.Headless.World.Save.Contract`; #916's populated encounter round-trip and v7/v1 absence migrations are pinned by `Test.Headless.World.Save.Compat` (including #917's v9→v10 and #2505's v11→v12 migrations), `Test.Headless.Location.SignificantContents`, `Test.Headless.Location.ContainerShells` and `Test.Headless.Location.Instance`, while exact same-page occupant resolution is pinned by `Test.Headless.World.Save.Integrity`. `pgsGenParams`' full fidelity (including `GeoTimeline`) is additionally cross-checked by chunk-regen determinism. | `wsToolModeRef`/`wsTimeScaleRef` reset-to-default is asserted live (`engine.getToolMode()`/`world.getTimeScale()` post-load) | `tools/world_check.py`, `tools/multiworld_save_probe.py`, `tools/location_content_probe.py`, `tools/river_naming_probe.py` |
...
| `portable-knowledge` | `SessionSnapshot.snapPortableKnowledge` (session-scoped: one map for the whole save, not a per-page slice) | `Test.Headless.Item.PortableKnowledge` — all four states through the real `encodeSessionSnapshot`/`decodeSessionEnvelope` pair, including a record whose two observation stamps differ, and the live `wmPortableKnowledge` owner through a real `handleWorldSaveCommand` capture, `loadWorld` bridge, `stageSession` scrub and `publishStagedSession` install | a save predating the component decodes to the EMPTY map (every crate never-inspected), a present-but-malformed or unsupported-version payload still fails the load, a record whose item instance is absent from the replacement session is SCRUBBED with a diagnostic during staging, publication REPLACES the live map rather than merging, and Exit to Menu empties it — all asserted by `Test.Headless.Item.PortableKnowledge` | `Test.Headless.Item.PortableKnowledge` (`--match "Portable container knowledge"`) |
