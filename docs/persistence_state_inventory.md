...
| `wsIdentityRef` | Persist exactly | — | `wpsIdentity` (#707 player-facing world identity; its optional `wiLanguage` language provenance rides along, #1092) | `tools/multiworld_save_probe.py` |
...
| `WorldPagesDTO` | `world-pages` (v5) | Persist exactly | the world page — page-set authority: identity (name, gloss, and the optional `LanguageProvenanceDTO` language provenance #1092 added), gen params (incl. the placed-location instance table and its page-local allocator, #911, each instance carrying the display name AND the optional English gloss #1101 renders from the page's own language, plus `wgpRiverNames` — #1102's per-page `RiverNamesDTO` table keyed by `GeoFeatureId`, each entry a river's native name and its English gloss), dates/clocks, map mode, per-page camera. v4 (pre-#1102) decodes through `migrateWorldPagesV4`, v3 (pre-#1101) through `migrateWorldPagesV3`, v2 (pre-#1092) through `migrateWorldPagesV2`, and v1 (pre-#911, per-chunk location flags) through `migrateWorldPagesV1` + `resolveLegacyLocations`. Every historical shape decodes with an EMPTY river-name table — a save written before rivers were named never acquires names, and none is inferred (#1102 requirements 5 and 6), while its rivers still carry ids derived from the timeline it already stores. Every historical shape likewise carries each stored location name across EXACTLY and produces an ABSENT gloss — a name rendered under one generator version is never re-derived (#1101 requirements 4 and 7) — and v2/v1/v90 additionally carry the page's own name/gloss across exactly with ABSENT provenance, never inferred (#915's precedent). |
