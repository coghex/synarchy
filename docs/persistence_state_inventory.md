...
| `wpsTransferOrders` | Persist exactly | — | #1246: this page's durable transfer orders, carried by the OPTIONAL `transfer-orders` component. Restored straight into `wsTransferOrdersRef`, verbatim — a dangling carrier/endpoint/item is logged and kept, never pruned and never a load failure. Appended for save v93 (the transitional in-memory bridge marker; the on-disk component carries its own v1). | `Test.Headless.World.TransferOrders` (`--match "persistence contract"`) |
...
| `unit_ai_claims` | `scripts/unit_ai_claims.lua` (registered from `scripts/unit_ai_save.lua`; the seven tables live in `unit_ai_dig`/`_chop`/`_construct`/`_farm`/`_repair`) | global (per-coordinate and per-item-instance work claims, session-wide) | Reset to default | — | reset hook (`registerResetHook`, not a save component — and deliberately NOT under the id `unit_ai`, which the persistent component above owns; `saveModules` refuses that collision). `claimsLib.resetAll()` empties all SEVEN transient coordination registries on every load, including a load with no data for this module family at all: the five coordinate claim tables (`digClaims`, `chopClaims`, `constructClaims`, `unitAi.till.claims`, `unitAi.plant.claims`) plus `repairClaims` and `repairPriority`. None of them lives in `aiState`, so none is reachable from the `unit_ai` component and none was reached by `unitAi.onSaveLoaded`. Never persisted, because a load REPLACES the session and both id allocators rewind (`nextItemInstanceIdRef` per `World/Load/Publish.hs`, `umNextId` per `unit_resources` above), so an inherited claim, claimant uid, or priority flag would attach to an unrelated entity — and the `now - c.at > timeout` expiry the six timed tables rely on cannot save them, because a loaded clock EARLIER than the writing session makes that delta negative (`repairPriority` has no timeout at all). Emptied IN PLACE, never rebound: the owning modules hold their tables as closure upvalues and `unitAi.till.claims`/`unitAi.plant.claims` are public fields. Content-data caches are deliberately excluded (`packBuildCache`, `foodDefCache` hold YAML-derived facts identical across sessions). `repairPriority` is classified transient rather than durable player intent — persisting it would need a typed reference kind, a `lua.unit_ai` version bump and a save-compat fixture; a player re-flags an item after loading. In-progress work still converges: a restored construction job the engine reports as `claimed` is adopted by `sweepConstructClaims`'s existing anonymous-timer path and released on the same timeout | `Test.Headless.Lua.UnitAiLoadReset` (`--match "unit AI load reset"`), `tools/construction_probe.py` |
...
| `WorldPagesDTO` | `world-pages` (v7) | Persist exactly | the world page — page-set authority: identity (name, gloss, and the optional `LanguageProvenanceDTO` language provenance #1092 added), gen params (incl. the placed-location instance table and its page-local allocator, #911, each instance carrying the display name AND the optional English gloss #1101 renders from the page's own language — and, since #1230, NO discovery margin: reveal is sight-based against the instance's own stored bounds, so v7 drops the field and `migrateWorldPagesV6` reads it off a v6 payload and discards it, plus `wgpRiverNames` — #1102's per-page `RiverNamesDTO` table keyed by `GeoFeatureId`, each entry a river's native name and its English gloss). Since #1104 all three of those name carriers — the page identity, each location instance, and each river name — additionally carry an OPTIONAL `EtymologySourceDTO`: the originating `NameExpr` (frozen as `NameExprDTO`, its own tag order) plus the `LanguageProvenance` that rendered it, so a stored name can be decomposed into roots and meanings without parsing it. Also: dates/clocks, map mode, per-page camera. v5 (pre-#1104) decodes through `migrateWorldPagesV5`, v4 (pre-#1102) through `migrateWorldPagesV4`, v3 (pre-#1101) through `migrateWorldPagesV3`, v2 (pre-#1092) through `migrateWorldPagesV2`, and v1 (pre-#911, per-chunk location flags) through `migrateWorldPagesV1` + `resolveLegacyLocations`. Every historical shape decodes with EVERY etymology source ABSENT — a save written before names recorded their expressions never acquires one, and none is inferred from the stored name, gloss, entity type, id, or content definition (#1104 requirement 1). Every historical shape decodes with an EMPTY river-name table — a save written before rivers were named never acquires names, and none is inferred (#1102 requirements 5 and 6), while its rivers still carry ids derived from the timeline it already stores. Every historical shape likewise carries each stored location name across EXACTLY and produces an ABSENT gloss — a name rendered under one generator version is never re-derived (#1101 requirements 4 and 7) — and v2/v1/v90 additionally carry the page's own name/gloss across exactly with ABSENT provenance, never inferred (#915's precedent). |
...
| `world-pages` | `PageSnapshot`'s identity/gen-params/dates/clocks/map-mode/per-page-camera fields (the identity's optional language provenance included, #1092; gen params include `wgpLocationInstances`, whose allocator is validated by `validatePages`, whose per-instance display name + optional gloss are #1101's, and which since #1230 carries no discovery margin at all, and `wgpRiverNames`, the per-page `GeoFeatureId`-keyed river name + gloss table #1102 added; the identity, each instance, and each river name each additionally carry #1104's optional `EtymologySource`) | `tools/persistence_contract_probe.py`, `Test.Headless.World.Save.Contract`; `wpsGenParams`' full fidelity (incl. `GeoTimeline`) is additionally cross-checked by chunk-regen determinism | `wsToolModeRef`/`wsTimeScaleRef` reset-to-default is asserted live (`engine.getToolMode()`/`world.getTimeScale()` post-load) | `tools/world_check.py`, `tools/multiworld_save_probe.py`, `tools/location_content_probe.py`, `tools/river_naming_probe.py` |
