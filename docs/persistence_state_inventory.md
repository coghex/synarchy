...
| `wsZoomLiveRef` | Rebuild | this page's own zoom cache + the generated chunks it was built from | #2485: the RGBA atlas pixels this page published, plus the colour palette they were generated with and the atlas layout, kept so an accepted terrain edit can regenerate one chunk's 32x32 block and republish the image the zoom renderer samples. Session-local presentation data, derived entirely from the page's own zoom cache: whichever path builds this page's atlas writes it (`World.Thread.Command.Init` on a fresh world, `World.Load.Stage` on a load), so a load re-establishes it from the pixels it stages rather than from anything a save could carry. `Nothing` for a page with no atlas of its own to patch — an arena, a page whose atlas was refused, or a loaded page that is not the session's atlas owner (#1670) — all of which render the zoom map per material instead. Nothing derives from it; it is never read as gameplay state | hspec `--match "solidification"`, `tools/fluid_reaction_visual_probe.py` |
