...
| `TutorialRegistry` (`trTree`, `trLoadFailed`) | `src/Tutorial/Types.hs:139` | global | Rebuild | `src/Engine/Asset/YamlTutorials.hs` | whole-tree validation at load (#957): unique ids, required presentation/evaluator fields, known kinds, resolvable references, valid kind/relationship combinations, exactly one root, no orphans, no cycles, one parent per objective. A file failing any check publishes nothing, drops any tree an earlier file published, and latches `trLoadFailed` so nothing republishes — so this slot is either the fully validated tree or explicitly `Nothing`, never partial and never dependent on directory read order. | hspec `--match "Tutorial definitions"` |
...
| `WorldPagesDTO` | `world-pages` (v2) | Persist exactly | the world page — page-set authority: identity, gen params (incl. the placed-location instance table and its page-local allocator, #911), dates/clocks, map mode, per-page camera. v1 (pre-#911, per-chunk location flags) decodes through `migrateWorldPagesV1` + `resolveLegacyLocations`. |
