...
**Narrative update (implementation, #2473):** the mover now charges a waypoint arrival its own distance and continues the tick from there, and the 0.1-tile arrival epsilon is replaced by a 1e-4 floating-point tolerance. All three reproduction schedules above now end at x = 1.4. The behavior recorded in this finding is the pre-repair behavior; the as-built contract, its constant-effective-speed scope, its exclusions, and its gates are in `docs/engine_contracts.md` §Movement tick: residual time across waypoints (#2473).
