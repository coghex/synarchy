...
with exactly **95** fields, `engineConfig` through `notificationOrder`,
...
| `content-registries` | Items, crafting, equipment, substances, infections, locations, loot tables, loot profiles, the tutorial definition tree, and the faction-tag catalogue: static, YAML-backed content registries loaded once and queried thereafter. |
...
| `world-sim-render-handoff` | `Engine.Core.Capability.WorldSim` — `WorldSimCapability` (12 fields, the world/sim half); `Engine.Core.Capability.RenderHandoff` — `RenderHandoffCapability` (10 fields, the coupled handoff half) | #893 (E5a) / #894 (E5b) |
...
| `Engine.Core.Capability.RenderView` (`RenderViewCapability`) | a strict subset — the 16 fields §5 records a `WorldThread`/`LuaThread`/`InputThread` reader or writer for (13 when #891 landed; `fpsRef` joined in #893, §7.2, `framebufferMinimizeGenRef` in #1693 and `maxImageDimensionRef` in #2020); **never `engineStateRef`** | any consumer, including every worker-thread one |
...
| `maxImageDimensionRef` | boot-process | `WorldThread` (`src/World/Thread/Command/Init.hs:322` and `src/World/Load/Stage.hs:155`, both through `Engine.Map.ImageAdmission.readMapImageCeiling`'s worker-safe `RenderViewCapability` projection), `LuaThread` (`src/Engine/Scripting/Lua/API/World/Lifecycle.hs`'s `world.init` and `world.checkMapImagePlan`, via the same helper) | `MainRender` (`src/Engine/Graphics/Vulkan/Init.hs:180`, the single write, in `initializeVulkanCommon` — the one funnel both the windowed and the offscreen device-creation paths pass through) | `IORef (Maybe Int)`, one `MainRender` writer during Vulkan init, read thereafter by workers; no read-modify-write anywhere | `Nothing` (`src/Engine/Core/Init.hs:362`) — no device exists at engine-init time | None | #2020. The device's real `maxImageDimension2D`, published so a worker can refuse a map image that cannot be created BEFORE it generates the pixels; `vulkanPDevice` itself is main-render-private `GraphicsState` (§3), so the worker gets the value and never the device. `Nothing` is the CORRECT steady state under `--headless`/`--dump`, which have no GPU: `Engine.Map.ImageAdmission.resolveMapImageCeiling` reads the boot mode, not this field, to tell "no ceiling applies" from "the query has not happened yet". |
...
rule: `worldPreviewRef` is a single slot consumed to `Nothing` by its
`MainRender` upload handler, `zoomAtlasDataRef` is a LIST its
`MainRender` handler drains whole (#2485 — runtime republication made a
single slot lossy), and `bloodDisposeQueue` is drained by its
`MainRender` consumer (`transient-handoff`);
...
| `zoomAtlasDataRef` | transient-handoff | `MainRender` (consumes for GPU upload) | `WorldThread` (enqueues — world init, an accepted reaction commit's atlas refresh `World.Thread.Command.Reaction.Zoom:162`, and load publish `World.Load.Publish:173`), `MainRender` (`Message.WorldTexture:223`'s `handleZoomAtlasUpload`, `atomicModifyIORef'` draining the whole list once dequeued) | `IORef [ZoomAtlasUpload]`, one entry per page still waiting | `[]` (`src/Engine/Core/Init.hs:407`) | None | Captures the exact `WorldState`s it belongs to at enqueue time (round 9 review, #763). A LIST rather than a single slot since #2485: an accepted solidification republishes its page's atlas at runtime, so two pages committing between render frames would otherwise have left one image silently overwritten. `queueZoomAtlasUpload` supersedes by PAGE id, which is what a world init uses — an init rebuilds ONE page, so another page's pending refresh is not obsolete and must not go with it. A load publish replaces the SESSION and so clears the queue whole, unconditionally: a session staged through the arena path carries no atlas at all, and clearing only when one is present would leave the outgoing session's images uploading into departed pages. |
...
| `materialRegistryRef` | session-replaced | `UnitThread` (`Unit.Thread.Movement`), `WorldThread`, `LuaThread` (`Engine.Scripting.Lua.API.World.Edit:86,211`, `Engine.Scripting.Lua.API.YamlTextures:350`), `MainRender` (`app/App/Dump.hs:152`, direct read while building the dump JSON) | `WorldThread` (per-world-init `atomicModifyIORef'` MERGING the live registry over a freshly-loaded `data/materials/*.yaml` base, live registrations winning on an id collision — `src/World/Thread/Command/Init.hs:182-184`, #2278; also load publish, `src/World/Load/Publish.hs:127`), `LuaThread` (`Engine.Scripting.Lua.API.YamlTextures:114-125` registers each material's physical properties from the same loaded YAML content) | `IORef MaterialRegistry`, multi-writer | `emptyMaterialRegistry` at engine boot (`src/Engine/Core/Init.hs:214`); per-world-init merge of the live registrations over `data/materials/*.yaml` (`src/World/Thread/Command/Init.hs:180-184`) | None | YAML-driven after all (corrected from an earlier draft of this row, per review) — rebuilt per world init/load from shipped YAML PLUS whatever `engine.loadMaterialYaml` registered live in this process, never built into the binary. World init MERGES rather than replaces (#2278), the same live-over-base precedence the load path applies (#763), so a custom id registered from a file outside `data/materials` survives a later world creation. |
...
| `gameTimeRef` | session-replaced | `InputThread` (`Input.Thread.Char:39`, timestamping a keystroke), `CombatThread` (`Combat.Resolution:120`, timestamping a combat event), `WorldThread` (`Thread.Command.Save.WriteWorld:88`, timestamping a save-triggered event), `UnitThread` (`Unit.Thread.Movement:61`, fall/landing timing), `LuaThread` (`API.Core:146`'s `engine.getGameTime`, direct query) | `UnitThread` (`Unit.Thread.unitLoop`, once per tick when unpaused; and once per Exit to Menu, `Unit.Thread.endSessionEpoch` restoring `Engine.Core.SessionEpoch.freshSessionGameTime` after the tick has drained both entity clears — #2291), `WorldThread` (load publish, `World.Load.Publish:110`, installing the save's own `sdGameTime`) | `IORef Double`, monotonic while unpaused | `0` (`src/Engine/Core/Init.hs:235`) | None | Persisted exactly (`sdGameTime`). Read from essentially every thread for event/log timestamping — enumerated by concrete role rather than `AnyThread`, since that identifier is reserved for a field with an explicitly documented unrestricted-access contract (§2.2), and this field has no such contract, just a wide but ordinary set of readers. |
| `enginePausedRef` | session-replaced | `WorldThread`/`UnitThread`/`SimThread` (skip simulated-state advancement while true), `CombatThread` (`Combat.Thread:87` — sleeps the tick, keeping queued events queued, rather than resolving combat while paused), `MainRender` (keeps rendering/input regardless), `LuaThread` (`API.Core:96`'s `isPausedFn`, direct query) | `LuaThread` (`API.Core`'s `setPausedFn`, `engine.setPaused`), `WorldThread` (load publish `World.Load.Publish:111`, always loads paused), `WorldThread`/`LuaThread`/`UnitThread` (through `World.Pause.imposePause`, which sets the flag for a `pause`-flagged notification category — the UnitThread reaches it since #2490, when `Unit.Thread.Command.Solidify` reports a solidification death and the player has set that category to pause) | `IORef Bool` | `False` (`src/Engine/Core/Init.hs:234`) | None | Persisted exactly; authoritative over any Lua-side copy. |
| `playerIntentGenRef` | session-replaced | `WorldThread` (`Thread.Command.Save.WriteWorld`'s `restoreAfterAutosave`, deciding whether an autosave may restore its own pre-request pause/time scale) | `LuaThread` (`Engine.Core.Capability.WorldSim.bumpPlayerIntent`, from `API.Core`'s `setPausedFn` on an APPLIED pause/resume and from `API.World.Clock`'s `worldSetTimeScaleFn` at request time — the only writer) | `MVar Word64`, monotonically increasing; the counter doubles as the MUTEX serializing a player transition against an autosave's compare-then-restore (`Engine.Core.Capability.WorldSim.withPlayerIntent` / `restoreIfPlayerIdle` / `withPlayerIntentHeld`), so a transition can neither slip in unseen after the comparison nor be overwritten by it. HOLDING that mutex is wider than writing the counter: every pause-epoch transition takes it through `World.Pause.withEpochLock`, so `WorldThread`, `LuaThread` and — since #2490's solidification deaths under a `pause`-flagged category — `UnitThread` all enter this critical section without bumping the value | `0` (`src/Engine/Core/Init.hs:238`) | None | #913. Deliberately NOT bumped by the engine's own writes to `enginePausedRef`/`wsTimeScaleRef` (auto-pause-on-save, load publish, a `pause`-flagged notification) — it records PLAYER intent, so that an autosave can tell "the player re-paused during my write" from "I paused it myself". #2490 added the UnitThread as a HOLDER of this mutex (a solidification death filed under a `pause`-flagged category), never as a writer of the counter; that call is deliberately made with `pageLifecycleLock` RELEASED, since no holder of that outermost boundary may take a second lock underneath it. Runtime-only, never serialized (see `docs/persistence_state_inventory.md` §1). |
| `enginePauseGenRef` | session-replaced | `WorldThread` (`Thread.Command.Save.WriteWorld`'s `restoreAfterAutosave`, deciding whether an autosave may close a pause epoch a second source still wants open), `LuaThread` (`Engine.Scripting.Lua.API.Save:481`'s `acceptSaveRequest`, snapshotting it into the `AutosaveRequest`) | `WorldThread`/`LuaThread`/`UnitThread` (all through `World.Pause.imposePause` — a `pause`-flagged notification category at `Engine.PlayerEvent.Emit:215`, reached from the world thread's discovery emits, the Lua thread's own emits and, since #2490, `Unit.Thread.Command.Solidify`'s solidification deaths; and an `engine.loadSave` acceptance at `Engine.Scripting.Lua.API.Save:662`) | `IORef Word64`, monotonically increasing; never read or written outside the `playerIntentGenRef` critical section — every epoch transition takes that mutex (`World.Pause.withEpochLock`) and so do both sites above, which is what makes the restore decision linearizable against a pause landing beside it | `0` (`src/Engine/Core/Init.hs:245`) | None | #1730. A pause EPOCH records no owner and `imposePauseHeld` is a complete no-op once the flag is set, so this counter is the only evidence that an engine source other than the running save still wants the game paused. Deliberately NOT bumped by the save's own pause (`World.Pause.reassertSavePause`, `acceptSaveRequest`) — a save may not count itself as a reason to decline its own restore — nor by the player's `engine.setPaused`, which `playerIntentGenRef` above already records, so a declined restore can name the right reason. Runtime-only, never serialized (see `docs/persistence_state_inventory.md` §1). |
| `pageLifecycleLock` | boot-process | `WorldThread`/`LuaThread`/`UnitThread` (only ever through `Engine.Core.Capability.WorldSim.withPageLifecycle`, in five roles. TRANSITION, on the world thread: `World.Thread.Command.Basic.handleWorldDestroyCommand` and `World.Thread.Command.Init`'s `registerPageIncarnation` — the shared registration step of both init paths. ADMISSION, on the Lua thread: the three entity-allocating verbs `unitSpawnFn`, `buildingSpawnFn` and `powerPlaceNodeFn`. COMMIT, on the unit thread: `Unit.Thread.Command.Spawn.handleUnitSpawnCommand` and `Building.Thread.Command.applyBuildingSpawn`, which revalidate the page-incarnation epoch their command carries in the same critical section as the insertion — and `applyBuildingSpawn` is shared, so #1602's bound commit takes it on the WORLD thread by that route too. SOLIDIFICATION KILL, on the unit thread: `Unit.Thread.Command.Solidify.handleUnitSolidifyOccupantsCommand` (#2490), the same fence shape for the reaction's occupant destruction. COHERENT READ, on the world thread: `World.Reaction.Occupants.snapshotSolidificationOccupants` (#2490), which writes nothing — it holds the mutex across two `readIORef`s, the unit roster and the authoritative positions, so no ADDITION can land between them; a removal still can, leaving a stale candidate the consumer's own roster recheck filters) | `WorldThread`/`LuaThread`/`UnitThread` (the same ten sites — the field carries no value, so TAKING the mutex is the whole of writing it, and that includes the one holder that only reads the records it protects) | `MVar ()`, a bare mutex with no state of its own. A page LIFECYCLE transition holds it across the `umNextId`/`bmNextId` readings, the immediate retirement of that incarnation's rows, the `wmWorlds` removal/replacement, and the `UnitClearPage`/`BuildingClearPage` enqueues carrying those readings as exclusive cutoffs. An entity ADMISSION holds it across its final live-page and page-binding revalidation, its id allocation, its footprint reservation where it takes one, and its queue insertion. A spawn COMMIT holds it across the re-read of the target page's incarnation epoch and the manager insertion itself — a fence rather than a check, since the epoch the command carries was verified at the top of a handler that does a great deal of work before it writes. A SOLIDIFICATION KILL (#2490) holds it on the same terms across the re-read of the reaction page's epoch, the roster-and-pose decision, the kills and the corpse-height correction; its player-event and injury rows are filed AFTER it releases, because a notification category the player has set to pause would otherwise route `emitEventFullOnPage` through `World.Pause.imposePause` and take the pause epoch's mutex underneath this one. A COHERENT READ holds it across the two `readIORef`s the reaction's victim snapshot takes, and what that buys is narrower than "one instant": no ADDITION can land between them, because the only site creating new unit membership in a live session — the spawn commit above — is a holder too. A page reincarnation cannot straddle them either, but because it ADDS nothing: it retires the outgoing incarnation's rows, and being a holder that removal lands on one side of the pair or the other. A unit REMOVAL is NOT covered, and this lock does not make one harmless: `UnitDestroy` bypasses it and retires the roster row and the sim row in two separate `atomicModifyIORef'` calls, in the same order the read takes them, so one landing between those reads is seen in both and enters the selection as a STALE CANDIDATE — filtered at the other end, by the consumer's re-read of the roster and the page epoch (the solidification kill's documented "Gone" case). Unit POSITIONS are not covered either — the movement tick and `UnitTeleport` write the horizontal position, the re-ground handlers and #2490's corpse settle write the vertical; one `readIORef` of that map is one coherent instant of every position at once, which is all that half claims | unheld — `newMVar ()` (`src/Engine/Core/Init.hs`) | None | #2476. The OUTERMOST coordination boundary of an admission: no holder may acquire another page or entity lock underneath it, and everything run under it is a `readIORef`, an `atomicModifyIORef'` transition or a non-blocking queue write. Deliberately NOT held across world generation — both init paths release it as soon as the replacement is registered, so a full `world.init` does not stall every Lua admission for the length of its worldgen. Alone in this capability it is `boot-process` rather than `session-replaced`: a session boundary and a load leave it exactly as they found it, because it is a critical section and not something a session owns. Runtime-only, never serialized (see `docs/persistence_state_inventory.md` §1). |
...
| `structureArtCatalogRef` | boot-process | `WorldThread` (`World.Render.CursorQuads`'s `renderCursorQuads` reads it through `rhStructureArtCatalogRef` once per frame into the `PlanWorld` snapshot both structure-ghost passes and the #2488 construction pass resolve against — the piece's art, and the construction frame its progress selects), `LuaThread` (`Engine.Scripting.Lua.API.StructureArt`'s `structureResolvePieceArtFn` and `structurePackKindBuildableFn` — `structure.resolvePieceArt` / `structure.isPackKindBuildable`, the synchronous unplaced-piece queries) | `LuaThread` (`Engine.Scripting.Lua.API.StructureArt`'s `structureRegisterPackArtFn` — `structure.registerPackArt`, one call per pack as `scripts/structures.lua` and `scripts/wire.lua` read `data/structure_packs/*.yaml`; and `Engine.Scripting.Lua.Thread.Dispatch`'s `LuaAssetFailed` handler, which fails the pack whose registered texture terminally failed to load), `WorldThread` (`World.Thread.Command.Edit.Structure`'s `handleWorldClearStructureCommand` — #2491's `noteMissingDestruction`, which records once per (pack, appearance) that a cleared piece's appearance declared no destruction clip) | `IORef StructureArtCatalog`, accumulate-only | `emptyStructureArtCatalog` (`src/Engine/Core/Init.hs:229`) | None | #1842. The unplaced-piece counterpart of `structureWallCatalogRef` above, and boot-process for the same reason: it is keyed by pack NAME and holds texture PATHS, neither of which a load publish's palette replacement can invalidate, so one registration per pack covers pieces designated later AND a session reloaded from a save. All or nothing per pack against an explicit declared-kind inventory, so half a pack can never be stored; a terminal texture-load failure makes the whole affected pack resolve nothing, recorded once per (pack, path) so the warning fires once per failed asset rather than per lookup. Since #2488 it also holds each authored APPEARANCE's ordered construction-frame sequence (`pkFrames`, keyed by variant + appearance so a variant never inherits the default's and an undeclared appearance resolves none), under the same all-or-nothing, idempotent, conflict-refusing rules — a declared frame that terminally fails to load fails its pack exactly as a static sprite does. Still accumulate-only, still keyed by pack NAME and holding PATHS, and still interning nothing: a construction frame is resolved per frame by `World.Render.StructureGhost.structureConstructionGhosts` and never placed, so it reaches neither `texPaletteRef` nor any per-frame cache of its own. Since #2491 it also holds each appearance's ordered DESTRUCTION clip (`pkDestruction`, same keying and same never-inherited rule, plus its own finite positive fps and a wall-family completeness rule), every authored VARIANT appearance's static sprite (`pkVariantArt` — the catalogue otherwise stores default art alone, and a variant piece whose appearance it has never heard of can neither resolve a clip nor have its missing one reported), a reverse index from each appearance's static texture PATH back to it (`sacAppearanceByPath`, which is how a placed piece — storing only palette ids and a z — resolves the appearance whose clip it should play; a path two appearances claim resolves nothing for either), and `pkMissingDestruction`, the per-appearance record of a gap already reported. That last field is the one thing the WORLD thread writes here, and it is bookkeeping rather than art: it adds no declaration and removes none, exactly as `pkFailures` records a load failure without changing what a pack declared. Registering or resolving interns nothing into `texPaletteRef` — knowing a piece's future art must not make a save carry palette entries for art nobody built (#1675), and #2491's capture reads only the two palette entries the cleared piece already stored. |

### `units-buildings-combat`

| Field | Lifecycle | Readers | Writers | Sync | Init | Shutdown | Notes |
|---|---|---|---|---|---|---|---|
| `unitManagerRef` | session-replaced | `UnitThread`, `CombatThread` (`Combat.Wounds.Tick`), `WorldThread` (also `Unit.Render`'s `renderUnitQuads`, reached via `updateWorldTiles` — not `MainRender`), `LuaThread` (`API.Units.Spawn:98`'s `unit.spawn`, direct def-existence check) | `UnitThread` (`Thread.Command.Lifecycle`, `Command.Pose`, and `Command.Solidify`'s corpse-height correction, #2490), `CombatThread` (`Combat.Resolution:323` — wound application, `Combat.Wounds.Tick:91` — periodic wound-tick outcomes, `Combat.Resolution.Wear:68` — weapon-wear mutation, all via `atomicModifyIORef'`), `WorldThread` (load publish, `World.Load.Publish:127`; and #2476's page/entity lifecycle transition — `World.Thread.Command.Basic`'s single-page destroy and `World.Thread.Command.Init`'s `registerPageIncarnation` for both init paths — which retires that incarnation's units under `pageLifecycleLock`, through the same `Unit.Types.Manager.retirePageUnits` body the queued `UnitClearPage` applies afterwards), `LuaThread` (`API.Units.Spawn:141`'s `unit.spawn`, direct unit-id allocation via `atomicModifyIORef'`) | `IORef UnitManager`, multi-writer | `emptyUnitManager` (`src/Engine/Core/Init.hs:215`) | None | — |
| `unitQueue` | boot-process | `UnitThread` (drains; `Unit.Thread.Command`) | `CombatThread` (`Combat.Wounds.Tick`, `Combat.Resolution.Events` — UnitKill/UnitCollapse), `WorldThread` (`Command.Basic`, `Command.Edit.Dig/Terrain`, `World.Reaction.Occupants` — one `UnitSolidifyOccupants` per tile the lava-water reaction turned to stone, #2490 — and `World.Load.Publish:210-220`'s `discardStaleQueues` on a load publish), `LuaThread` (`API.Units.Spawn`) | `Q.Queue UnitCommand` | `Q.newQueue` (`src/Engine/Core/Init.hs:216`) | The combat thread (a producer) is shut down **before** the unit thread (its consumer) — `app/App/Graphical.hs:72-76`: "Combat first: wound ticks enqueue UnitKill/UnitCollapse onto the unit queue, so the producer has to stop before the consumer... is torn down" | Deliberate shutdown ordering; see the identical rationale on `combatQueue`. |
| `utsRef` | session-replaced | `UnitThread` (`Unit.Thread`, `Thread.Command`, `Thread.Movement`, `Thread.Command.Spawn/Lifecycle/Pose`), `WorldThread` (`Thread.Command.Save.WriteWorld:101`, save capture; and `World.Reaction.Occupants`, which resolves the occupants of a solidifying cell from the AUTHORITATIVE positions rather than the once-per-tick `umInstances` mirror, #2490), `LuaThread` (`API.Units.List:142`'s `unit.getInfo`, direct query) | `UnitThread`, `WorldThread` (load publish, `World.Load.Publish:128`) | `IORef UnitThreadState`, single-WRITER by `UnitThread` outside a load publish, read by the world and Lua threads where they need the authoritative position (per the field's own doc comment) | `emptyUnitThreadState` (`src/Engine/Core/Init.hs:217`) | None | — |
...
| `treatRNGRef` | boot-process | `LuaThread` (`API.Units.Medical` — the dressing attempt cycle and, when it is not yet rolled, the medic's capability stat) | `LuaThread` (`API.Units.Medical`) | `IORef StdGen`, SINGLE-writer by contract — one module, on one thread, never re-entrant | `Random.newStdGen` (`src/Engine/Core/Init.hs:422`) | None | #2297. A treatment is decided and applied in one unit-manager transaction that can still refuse at commit time, so its generator must be claimable without advancing anything a refusal would have to unwind. On the four-writer `statRNGRef` that is impossible: claiming atomically IS advancing it, and claiming by a plain read lets `Combat.Wounds.Tick`'s own `splitGen` receive the very generator the treatment is using. Explicitly non-deterministic across runs, like `statRNGRef` — not save-seeded. |
| `buildingManagerRef` | session-replaced | `WorldThread` (`Render.CursorQuads`, `Thread.Power`, `Thread.ItemTemp`), `LuaThread` (`API.Power`, `API.Save`, `API.Buildings.Selection`'s `building.getSelected`), `UnitThread` (`Building.Thread.Command:31`'s `handleBuildingCommand`, drained on the unit thread) | `UnitThread` (via `Building.Thread.Command`, drained on the unit thread), `WorldThread` (load publish, `World.Load.Publish:126`; `World.Thread.Command.BoundSpawn`'s stale-binding drop, which retires the placement's footprint claim, #2326; and #2476's page/entity lifecycle transition — `World.Thread.Command.Basic` and `World.Thread.Command.Init`'s `registerPageIncarnation` — which retires that incarnation's buildings, destruction effects and footprint claims under `pageLifecycleLock`, through the same `Building.Types.retirePageBuildings` body the queued `BuildingClearPage` applies afterwards), `LuaThread` (`API.Buildings.Selection:46,59`'s `building.select`/`building.deselect`, direct `atomicModifyIORef'`) | `IORef BuildingManager` | `emptyBuildingManager` (`src/Engine/Core/Init.hs:219`) | None | "Building" is a domain, not a thread — its commands are drained on `UnitThread` (`Unit.Thread` imports `Building.Thread.Command.processAllBuildingCommands`). |
...
`infectionManagerRef`, `locationDefsRef`, `lootTableRegistryRef`,
`lootProfileRegistryRef` and `tutorialRegistryRef`, reader and writer
alike still take
...
| `lootProfileRegistryRef` | boot-process | `LuaThread` (`loot.profile`, `loot.listProfiles`, `loot.simulate`) | `LuaThread` (content load, `engine.loadLootProfileYaml`) | `IORef LootProfileRegistry` | `emptyLootProfileRegistry` (`src/Engine/Core/Init.hs:468`) | None | Loot PROFILES, not loot tables (#2499, epic #1231 PLC-12): a table is one weighted draw, a profile rolls every entry independently against its own absolute chance. Its writer (`src/Engine/Scripting/Lua/API/LootProfiles.hs`) is the one module in this group that takes BOTH records — this one to write its own registry, and `ContentRegistriesViewCapability` to READ `itemManagerRef` for the load-time item-id check (D-20), so it never holds write authority over items. `loot.simulate` (#2502, PLC-13) is the first reader that ROLLS a profile: it resolves the id here and realizes `sampleCount` shells against the read-only item view, using only call-local allocators and generators. |
| `factionCatalogueRef` | boot-process | `None` (no reader verb yet — FTS-2 (#2506) lands the authority and its validation; the live `UnitInstance` profiles that will consult it are FTS-3 (#2515), and requirement 9 forbids a faction query verb in this slice. `Engine.Scripting.Lua.API.Units.Yaml:116` READS it inside the same `LuaThread` content load that writes it, to resolve each unit definition's `faction_tags:`) | `LuaThread` (content load, `engine.loadFactionYaml`, `src/Engine/Scripting/Lua/API/Factions.hs:96`) | `IORef FactionCatalogue` | `emptyFactionCatalogue` (`src/Engine/Core/Init.hs:482`) | None | The declared faction-tag vocabulary and the authored base relation table behind `Unit.Faction.Profile`'s policy (#2506, epic #2496). Loaded BEFORE `data/units` in both startup profiles, because a unit definition naming a tag the catalogue does not declare refuses that whole file (D-30). Declaring a tag does not close the namespace: a runtime-minted valid tag still evaluates under the same precedence (D-12), so this registry restricts authored data rather than the model. |
...
| `eventStoreRef` | session-replaced | `LuaThread` (`API.PlayerEvent`'s `readEventLog` — `engine.getEventLog()`, the event-log panel's query — and `readEventLogProgress` — `engine.getEventLogProgress()`, the #1714 rows-plus-high-water read, one `readTVarIO` so the pair cannot straddle a commit) | `WorldThread` (`World.Thread.Discovery`'s `emitEventFullOnPage`, `World.Thread.Command.Save.WriteWorld`'s `emitEvent`, and load publish `World.Load.Publish.resetTransientState`, rows reset to empty, counter kept), `UnitThread` (`Unit.Thread.endSessionEpoch`, the Exit-to-Menu boundary — rows reset to empty on the same terms, counter kept, #2291; and `Unit.Thread.Command.Solidify`'s `emitEventFullOnPage`, one row per unit the lava-water reaction entombed, #2490), `LuaThread` (`API.PlayerEvent`'s `emitEvent`/`emitEventAt`/`emitEventFull` — `engine.emitEvent`/`emitEventAt`/`emitEventForUnit` — and `API.Save`'s save/load-lifecycle emits) | `TVar EventStore`, multi-writer STM, ~1000-entry ring of `StoredEvent` rows plus the `esNextSequence` mutation counter (#1714) — one ref so a row's sequence is stamped in the same atomic write that commits the row, and so the counter outlives BOTH session-boundary row resets (`Engine.PlayerEvent.clearEventStoreRows`, called by the load publish and by the Exit-to-Menu boundary) | `newTVarIO emptyEventStore` (`src/Engine/Core/Init.hs:269`) | None | Explicitly session-only, never serialized. One `Unit.Thread` call site emits a player event since #2490 (`Unit.Thread.Command.Solidify`'s `reportKills` phase, the unit-thread half of the lava-water reaction's occupant destruction — it is there, and not on the world thread that commits the stone, because only the unit thread can say whether a named occupant was still alive, and it runs AFTER its `commitKills` phase releases `pageLifecycleLock`, since a `pause`-flagged category would otherwise take the pause epoch's mutex underneath that one); no `Combat.Thread` call site does. `Engine.PlayerEvent.Emit`'s module comment, `EngineEnv`'s own field doc and `Engine.Core.Init`'s seeding comment each name the live emitter set: #898 corrected all three to state the STM primitive's any-thread safety separately from the call sites that actually exist, and #2490 added the unit thread to all three. |
...
| `render-gpu-asset` | *(none — migrated by #891 (E3): all 45 former entries now reach their render fields through `Engine.Core.Capability.Render` (the `MainRender`-only 22-field record) or `Engine.Core.Capability.RenderView` (the worker-safe view that never carries `engineStateRef` — 13 fields when #891 landed, 16 since #893 added `fpsRef`, #1693 `framebufferMinimizeGenRef` and #2020 `maxImageDimensionRef`), per §3.1; none of them holds unrestricted `EngineEnv` access any more, and no module remains whose dominant field usage is this capability)* | §7.2 |
...
- **What landed:** two projection modules, both total, one-way, and
  ...
  - `Engine.Core.Capability.Render` exports `RenderCapability` over
    exactly the 22 fields of §5's `render-gpu-asset` table plus
...
- **Three steering-text corrections rode along**, each a §5 row this
  ...
  separately from the call sites that actually exist. Those were the
  world and Lua threads when #898 wrote this; #2490 added a unit-thread
  emitter (`Unit.Thread.Command.Solidify`, one row per unit the
  lava-water reaction entombed) and updated all three texts with it.
  No combat-thread emitter exists.
- **Enforcement:** the §6 ratchet (`TEMPORARY_CEILING`'s
