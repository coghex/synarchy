...
| `pageLifecycleLock` | boot-process | `WorldThread`/`LuaThread` (only ever through `Engine.Core.Capability.WorldSim.withPageLifecycle`: on the world thread `World.Thread.Command.Basic.handleWorldDestroyCommand` and `World.Thread.Command.Init`'s `registerPageIncarnation` — the shared registration step of both init paths; on the Lua thread the three entity-admission verbs `unitSpawnFn`, `buildingSpawnFn` and `powerPlaceNodeFn`) | `WorldThread`/`LuaThread` (the same six sites — the field carries no value, so TAKING the mutex is the whole of writing it) | `MVar ()`, a bare mutex with no state of its own. A page LIFECYCLE transition holds it across the `umNextId`/`bmNextId` readings, the `UnitClearPage`/`BuildingClearPage` enqueues carrying those readings as exclusive cutoffs, and the `wmWorlds` removal/replacement; an entity ADMISSION holds it across its final live-page and page-binding revalidation, its id allocation, its footprint reservation where it takes one, and its queue insertion | unheld — `newMVar ()` (`src/Engine/Core/Init.hs`) | None | #2476. The OUTERMOST coordination boundary of an admission: no holder may acquire another page or entity lock underneath it, and everything run under it is an `atomicModifyIORef'` transition or a non-blocking queue write. Deliberately NOT held across world generation — both init paths release it as soon as the replacement is registered, so a full `world.init` does not stall every Lua admission for the length of its worldgen. Alone in this capability it is `boot-process` rather than `session-replaced`: a session boundary and a load leave it exactly as they found it, because it is a critical section and not something a session owns. Runtime-only, never serialized (see `docs/persistence_state_inventory.md` §1). |
