...
-- Transient stripping (issue #761 requirement 13/14, split out in
-- #2553 to stay under this file's line budget, #538): WHICH per-unit
-- aiState fields a snapshot drops and on what grounds -- the scratch
-- candidates, an in-flight order's trip bookkeeping, the work
-- accumulators and their clocks, and the nested ids that would
-- otherwise ride the wire untyped. unit_ai_save_transient.lua states
-- each reason where it applies.
local snapshotUnitState =
    require("scripts.unit_ai_save_transient").snapshotUnitState
