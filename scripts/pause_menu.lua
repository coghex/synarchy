...
function pauseMenu.onSave()
    ...
    if worldManager.currentWorld then
        -- Pause through the module first so the player's own pause is
        -- what the log and the mirror record, and so the game is
        -- already stopped while the (asynchronous) save runs. The save's
        -- own acceptance pauses too, and since #1599 either route keeps
        -- enginePausedRef and the world clock (wsTimeScaleRef) in sync
        -- and hands the player's chosen speed back on resume -- this
        -- call is no longer what rescues the speed, it is just the
        -- explicit player-facing pause. (Opening this menu does not
        -- itself pause; the game runs underneath.)
