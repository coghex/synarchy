...
Not preserved (transient UI / in-progress state, by design): current selection (cursor selection, `umSelected`, `bmSelected`), build-tool placement mode (`buildTool.state` + `buildingGhostRef`). A fresh-session load lands in default UI state — the player re-selects, re-enters the build tool, etc. Within-session loads keep the singleton Lua state. The outer `sdToolMode` (which tool is active in the toolbar) IS preserved.
