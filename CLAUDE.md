...
- Those budgets validate GENERATED ARTIFACTS only. The runtime bound of
  one atlas upload and one texture handle per animation belongs to the
  Hspec group `Unit.Atlas.Load — the real unit registration boundary`
  (#2217); a green strict run is not evidence about it.
