...
class FakeClaim:
    """A `claim_lease.Claim` stand-in with the same ownership surface."""

    def __init__(self, probe: str = PROBE, *, token: str = "token-1",
                 lose_on: str = "", release_error: str = "") -> None:
        self.probe = probe
        self.token = token
        # Read by the REAL `claim_lease.Renewer`, which the orchestrator
        ...
        self.lease_seconds = claim_orchestration.MIN_ORCHESTRATION_LEASE_SECONDS
    ...
    def release(self) -> bool:
        if self.release_error:
            raise claim_storage.ClaimError(self.release_error)
        self.released = True
        return True
