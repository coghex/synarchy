...
| `content_registry_probe.py` | #890 | worldgen | The `content-registries` capability's eight publicly reachable registries end-to-end: each written through its public `load*Yaml` verb and read back through its public query (substance, item, equipment class, infection, recipe/repair, location def, loot table incl. `loot.roll`, and loot profile incl. `loot.profile` / `loot.listProfiles`), post-boot re-loading replacing by id rather than freezing or duplicating, and `world.listPlacedLocations` joining real placements against the location-def registry. |
...
| `fluid_reaction_probe.py` | #2485 | worldgen (two real engine boots, isolated resource root) | The lava-water reaction product is DURABLE. Generates a real worldSize-8 page (never an arena — `docs/headless_console.md` NB #365 records that loading a save containing one hangs the world thread), finds a flat dry land pair, drives a real contact with `world.setFluidTile`, and waits for the lava column's TERRAIN top to rise — which only the world thread's reaction commit can do, since neither fluid edit raises terrain and a fluid writeback never touches a column's materials. Records the height and the product material through `world.getMaterialAt`, saves, QUITS, and boots a second engine that loads the save and asserts the same height and the same material at the same tile. An ocean contact is used so D-5's first clause picks the product without depending on where generation put sea level. |
| `fluid_reaction_visual_probe.py` | #2485 | offscreen, needs a GPU (one boot, worldSize 8) | An accepted solidification reaches BOTH live presentations with no page reload. The contact is visually silent by the owner's decision, so the stone itself is the presentation: the detailed tile render (which rebuilds quads from `wsTilesRef`) and the zoom map (which does NOT — its renderer samples a precomputed atlas whose terrain pixels are produced once at page init, so only #2485's regenerated block and republished atlas can change it). Reaches the gameplay surface through `uiManager.showMenu('world_view')` with a pinned small world rather than the create-world screen's own widgets, pins camera, zoom, time scale and sun angle, and pauses around every capture. Each frame is paired with an immediate re-capture as the byte-stability control, so the before/after difference cannot be renderer noise; the detailed half is graded INSIDE the solidified tile's own screen box, and the ZOOM half inside that tile's own ATLAS pixels — located through `world.zoomTileRect`, which runs the map's own projection forwards (the chunk rectangle the atlas bakes over, the wrap offset the quads apply, the inverse-isometric texel transform the pass colours through) rather than the detail hit test, which walks terrain z and would hand back a padded box taking in the tile's neighbours. A first contact in the same chunk establishes the zoom baseline, because neither `world.addTile` nor `world.setFluidTile` refreshes the atlas and the refresh rebuilds a block from the whole edit log — so only what the measured contact adds is attributable to its own stone; the fluid is settled before that baseline is published, since ordinary writebacks never touch the atlas. Asserts the page identity, active page and `engine.getLoadStatus()` are unchanged, so "without reloading" is checked rather than assumed. Both contact sites are ice-free, checked through `world.getIceAt`: the zoom map draws an iced tile as snow whatever material lies under it, so a solidification there changes no zoom pixel and grading such a tile would measure nothing. |
...
| `offscreen_probe.py` | #650 | offscreen (needs a GPU) | `--offscreen` render mode end to end: windowless Vulkan boot + real UI flow, non-blank screenshot capture, F2 input injection driving the UI, parallel instances on separate ports, and (unless `--skip-worldgen`) a full click-to-generate-world path to the in-game HUD. The generated world is **pinned** (#2166): seed, world size and plate count are set through the create-world screen's own controls before Generate World is clicked, `world.getSeed` confirms the world that came out, `--seed` overrides the default, and every run prints a one-line `FIXTURE` identity. A bounded search that finds no buildable position prints every candidate it rejected (resolved tile, `canPlaceAt` verdict and reason, full `remoteCheck` classification) and is reported as a `[SETUP]` fixture failure — counted separately from product assertions in the closing summary, still non-zero at exit. `--force-search-miss` exercises that path without editing the file. |
...
| `preview_probe.py` | #886, #887, #888, #1907, #2089 | hidden, non-activating real window (still needs GPU; no offscreen variant) | `--preview` real-boot browser (`SYNARCHY_PREVIEW_HIDDEN=1`, a fresh hidden window per target — one per fixed phase, one per shipped unit, one per swept category, one per zoomed display kind — and one retained log per boot, #1763). Since #2089 the file is the facade and six independently runnable families own the scenario bodies under `tools/preview/`: `simple` (phases 1-2), `units` (3, 4, 4b), `buildings` (5-7), `dispatch` (8-9), `zoom` (11), `audio` (synth/file playback, reload, and the visual footer with forced-null output), with the shared mechanics (the one `boot_preview` launcher, dumps, input/geometry helpers, expectations, the trimmed-loading check) in `preview/harness.py`. `--only <family>` runs just that family's boots; with no selector every scenario runs exactly once in the facade's `FAMILIES` inventory order, and one aggregate PASS/FAIL closes either run. The family modules are LIBRARIES, not probes — `preview` stays the single registration — and `tools/test_probe_boot_logs.py` pins the dispatcher, the selectors, the pre-boot rejection of an unknown family and the per-boot logs from that inventory, GPU-free. Simple family (#886): simple-category list mode reports boot profile `"preview"` and the parsed target, simple-category list mode reports boot profile `"preview"` and the parsed target, its discovered entries (`require("scripts.preview_manager").dump()`) match an independently-computed filesystem expectation, the first entry auto-selects and resolves, clicking a different row (located from the dump's own row bounds) changes selection, wheel input changes the reported scroll offset, and a grow/shrink resize reflows without overflowing; focused item mode has no list while its texture resolves; every requested texture path stays under the browsed category's root. Units family (#887): the animation list matches a filesystem-derived expectation exactly and in order, the default selection is `idle`/south, effective fps/loop match `data/units/<name>.yaml`, all eight direction cells appear in the game's order with the western three reporting their real mirror source, the frame index advances over wall time, a dump-located row click switches clips and a dump-located mirrored-cell click enlarges that direction, a resize preserves animation/direction/scroll, and a PROMOTED tree (`tiller`, declared under `asset_units:` by #1257 and under `units:` since #1261) reports its DECLARED fps=8/loop=true/flip=true with W/SW/NW mirrored from their eastern counterparts and is atlas-backed like any other unit. #1261 adds a roster phase: one boot per remaining declared unit (the list read from `data/units/`, never written in the probe), asserting the animation list still equals the filesystem expectation, every animation reports atlas storage naming its own compiled atlas from the index, and nothing outside that unit's own directory loaded. Buildings family (#888): the mixed animation-directory + loose-static entry list matches a filesystem+YAML expectation exactly and in order with each row's own static/animation identity, the default selection is the DIRECTORY holding `state_animations.built`'s declared frames (`idle`, not the YAML's `portal-idle` name), its fps/loop come from that entry, the frame index advances, a resize preserves selection/scroll, and a dump-located static-row click selects it and exposes no playback; a building with no `built` state falls back to its `sprite` and still recognizes a YAML-less `demolish/` folder by the numbered-frame convention at fps=8/loop=false; a building with no YAML at all (`dungeon_1`) surfaces its `damaged/` subtree as ordinary statics and defaults to its first entry; Dispatch family (#888): `flora/<name>` and `structures/wire` dispatch into the shared simple browser rooted at the item folder, and the canonical sweep proves every category dispatches with no `placeholder` mode left anywhere. Zoom family — centered bounded zoom (#1907): one boot per display kind (bare list, focused item, unit enlarged, building, flora item, structure item), driving the REAL wheel pipeline with `input.moveMouse` + `input.scroll` over the dump-reported zoom REGION rather than any hardcoded coordinate — a session starts at multiplier 1, delta MAGNITUDE is honoured rather than reduced to a sign, both limits clamp exactly at 1/8 and 1 with further input consumed and stable, the rendered dimensions are one eighth of the fitted ones at the floor and exactly the fitted ones back at the ceiling, the sprite stays centered and wholly inside its region throughout, plain and a really-held-Shift wheel behave identically, a wheel over a located list row moves the list and never the zoom while a wheel over the pane moves the zoom and never the list (including once saturated), the object-identity rule resets on a different BARE-category texture and preserves for a unit animation / building entry / flora piece, and a resize preserves the multiplier while recomputing the region. |
...
| `scene_primitives_probe.py` | #2192 | offscreen (main menu only, needs a GPU) | **Scene primitives reach the frame.** `engine.spawnText` and a UI-layer `engine.spawnSprite` over the real main menu, each attributed to its OWN bounded screenshot region (a single changed frame holding both would pass with one primitive missing), after two idle captures prove those regions static; then every mutation verb on its own frame — `setText`, `setVisible` off/on (the hidden region returns to the baseline while the other stays drawn), `setPos` (the sprite leaves one region and appears in another), `setSize`, `setColor`, and `destroy` for each — proving the route is rebuilt every frame. The exact per-layer merge and equal-layer order the recorder consumes is the CI-gated pure hspec group `frame layer assembly`. |
...
| `startup_asset_logging_probe.py` | #1930 | offscreen boot, both profiles (needs a GPU) | **Startup YAML logging ownership.** Boots `--offscreen` normal and `--offscreen --arena` with `ENGINE_DEBUG=Asset`, waits on `startupLoader.isDone()` (the two profiles finish through different code — normal drains across frames under `loadingScreen.update`, arena runs `runAll()` synchronously inside `uiManager.checkReady`), and reads the real log: exactly the 13 / 12 expected aggregates located by their stable family identifier and in queue order, no flora aggregate or `loadFloraYaml` call in arena, no `load*Yaml: loaded` line left at Info for any of the thirteen bindings, per-file Debug detail carrying each file's full `data/<family>/` path and the authoritative count that binding returned, and every aggregate equal to the sum — and the file count — of its own family's observed per-file lines. One check exists purely to prove the matcher discriminates: the unrelated startup lifecycle Info lines must be present and uncounted. Offscreen rather than headless because a `--headless` boot never reaches `scripts/loading_screen.lua` or `scripts/ui_manager_boot.lua` (which gates on `fontsReady`), so the startup asset queue never runs there and every check would pass vacuously. Both halves of the contract are gated on CI by the pure hspec group `Startup asset logging`. |
...
| `structure_construction_probe.py` | #2488 | offscreen arena (needs a GPU) | **Structure construction frames, pixels.** The one claim the hspec group "structure construction frames" cannot make: `renderFlagLifecycleAlpha` is read in the FRAGMENT shader, and a headless run has no fragments. Registers a fixture pack through `structure.registerPackArt` whose art is built to make the shader's decision legible — a facemap with a painted top band, a zero-RGB band and a transparent remainder, against a construction frame carrying a distinct primary in each band plus a fully transparent one — then designates, PAYS (an authored empty bill, which is a real paid state) and advances one site to mid-progress, places the same art one tile over as an ordinary UNFLAGGED static piece, and reads the pixels: the frame's out-of-silhouette pixels are present, the static control's are not, a transparent frame pixel paints nothing, and a painted mask, a zero-RGB mask and no mask at all light the frame identically. A fifth claim covers scene opacity: at a zoom inside the world's fade band, band C comes back dimmer against its own baseline at that zoom, so the tint's alpha still multiplies the flagged quad. Attribution is by DIFFERENCE **and** POSITION: the captures share one camera pinned before the first of them and the charged unit is spawned before it too, so a difference set narrows to what a step added — but `addJobProgress` also stamps the D-18 corner slope and re-meshes the tile, so terrain legitimately lands in that set, and every colour claim is therefore also placed: bands A and B are inside the facemap silhouette and drawn either way, and their measured screen spans against their known canvas columns give the ruler every other band is asserted against. Each claim is gated on its own difference set being non-empty, so "no cyan" cannot mean "nothing was placed". Each capture removes its output file first and checks the engine's own reply, so a screenshot that never happened cannot be read back as the frame under test. Fixture art is generated into a gitignored directory under the resource root — a declared frame path that escapes it is refused at registration — and removed on the way out. |
...
| `tutorial_probe.py` | #922, #2301 | worldgen (four engine boots, two real save/loads) | Drives the shipped preparation-to-encounter frontier from real gameplay: portal placement, FOV water discovery and radio sharing, stepwise supplies on one acolyte, durable completion and reversible checks. Preparation now reveals Confront before its supply rows and stays active until that child completes. Separate pre-latched and fresh-process reload legs verify that no renderer means no presentation, explicit test acknowledgement retires hide suppression, and retired portal/water ancestors do not return. Injects no tree and stubs no predicate. The facade alone owns CLI, boot/quit and save-slot cleanup; contracts, ordinary/sticky stages and shared harness remain in their split owners (#2145). |
| `tutorial_hud_probe.py` | #960, #2301 | worldgen, `--offscreen` (needs-gpu, manual-only) | Renders the real tutorial HUD: toggle clicks, transparency, long-list wheel scrolling and terrain pass-through. Pixel attribution proves the caption and every shipped row fit the panel/framebuffer; stable registry-backed captures cover all nine authored rows at seven depths. An injected preparation-only tree retains the #996/#1941 retirement proof. A separate shipped-tree batch captures Recover, Secure and Clear checked together on an open panel before renderer acknowledgement retires them; closed/no-frame retention is covered by the headless gates. Captures and the presentation handshake occur inside one Lua chunk to prevent interleaved retirement. A real save/load restores the preparation-to-encounter continuation without resurrecting retired ancestors. Containment is evidence for the size actually run. |
...
├── preview/                (family owners + shared harness behind preview_probe.py, #2089 — libraries, not probes)
...
### Audio checks and the forced-null integration probe

`audio_null_probe.py` is a CI-eligible, window-free, speaker-free smoke probe. It
uses a private resource root and generated decoder fixtures, boots the actual
headless executable and asserts miniaudio's null backend. Its nine checks cover
sample/synth coexistence, callback progress, spatial listener publication,
player pause/resume, live/saved volumes, bounded unknown-ID diagnostics, session
replacement and clean process exit. It uses one port; default 9187. Fixture
config and catalogs are copies, and other resources are read-only symlinks.

```bash
python3 tools/audio_null_probe.py --describe
python3 tools/run_probes.py --only audio_null --exact
```

The native numerical/failure gate is `python3 tools/test_audio_native.py --sanitize`.
It validates the vendored hashes, compiles the production mixer with address and
undefined-behavior sanitizers, and exercises deterministic decode/DSP/voice/ring
and null-only device recovery. Failure injection is confined to this standalone
test build. `--build-dir /tmp/synarchy-audio-native` retains its binary and compiler
commands; vendor objects are reused only for the same content/compiler/flags.

`python3 tools/test_audio_build_dependencies.py` compiles and runs the real Setup
header-dependency hook against private timelines. Both commands run in CI's
Cabal-equipped job and the local CI mirror. They are focused commands, not a
request to run the entire local gate. See [audio runtime](../docs/audio_runtime.md)
for configuration/status interpretation and [fixture provenance](../test-headless/data/audio/README.md)
for regeneration. Physical-device listening is a separate manual acceptance step.

`audio_manual_probe.py` is manual-only (`needs-gpu`, `targeted`). Run it directly:

```bash
python3 tools/audio_manual_probe.py --offscreen-check --port 9188
python3 tools/audio_manual_probe.py --interactive --port 9188
```

The first mode renders the real Settings → Audio screen, captures its three
sliders and runs the sample/synth/spatial/loop/pause sequence through forced-null
output. The second explicitly opens a visible game and speakers/headphones;
use a terminal, comfortable device volume and an owner-authorized visible launch.
It starts at Master 50%, World/Interface 70%, prints negotiated backend/device/
rate/period and ring latency, and asks for listening and optional device-change
verdicts. No operating-system device switch is automated. Both modes retain a
JSON evidence file, PNG and engine log under `/tmp/synarchy-audio-*` by default,
and clean up their own process and private resource root. Output to a null
fallback or skipped recovery remains distinguishable from physical acceptance.
An invocation without an explicit mode exits before launching anything; use the
direct command, since the aggregate runner does not choose an audible mode.
