...
| `transfer_context_menu_probe.py` | #1014 | worldgen, `--offscreen` (needs-gpu, manual-only) | The "Transfer" context-menu entry end to end: a real right-click on a real built storage building and on a real technomule, with the "Transfer" row located through `ui.dumpWidgets()` by its visible label (never a hardcoded coordinate), the existing "Contents"/"Info" rows still present alongside it, `debug.drainActionOutcomes()` confirming the click routed through the context-menu handler and never fell through to a move order, and a real click on the row producing a `scripts.transfer_session` session naming the exact source unit and receiver identity for both receiver kinds. |
