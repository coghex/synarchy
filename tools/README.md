...
prime is recognised over the full Haskell identifier set — letters,
digits, `_`, `'` **and combining marks**, which GHC accepts (issue
#7650) and Python's `\w` does not match. Read with a narrower class, the
prime of `π` + U+0301 opens a char literal that eats the quote after it,
