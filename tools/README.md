...
| `persistence_contract_sweep.py` | #767 | worldgen (size 64, three real engine boots, isolated resource root) | Broader persistence contract sweep: the SAME three-cycle fresh-process structural comparison against a real generated-world representative scenario (a built craft station running a bill, a mine designation, a world identity); cross-references (does not re-run) the domain-specific and assembled-failure-contract probes. |
