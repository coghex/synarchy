...
asserting for each refusal that nothing was written. Every cross-field rule
gets three cases: a rejecting fixture that is still SCHEMA-valid, so only that
rule can reject it, driven through `--record`, a policy update, `--seed` and
`--validate`; the legitimate flows it must not over-reject (an empty census, a
TIMEOUT run carrying a FAIL check, a harness error that completed no run at
all, cohort rollover, promotion); and a mutation check that lifts that one
rule out of the production rule set and requires its own fixture to be
accepted again.
