...
def cohort_evidence(cohort, what: str) -> tuple:
    ...
    """
    statistic, anchor = cohort_statistic(cohort, what)
    completed = 0
    timeouts = 0
    worst = None
    earliest = None
    for position, sample in enumerate(cohort["samples"]):
        ...
        timeouts += require_count(sample.get("timeout_count"),
                                  f"{where} `timeout_count`")
...
def unresolved_attempts(census, probe: str, cohort_commit: str,
    ...
    for attempt in attempts:
        ...
        if attempt.get("accepted") is True:
            continue
        try:
            commit = require_commit_identity(
        ...
        if commit is not None and commit != cohort_commit:
            continue
        try:
            stamp = parse_timestamp(attempt.get("timestamp_utc"),
                                    "attempt `timestamp_utc`")
        except CensusError:
            stamp = None
...
def promotion_row(entry, *, now, stale_after_seconds) -> dict | None:
    ...
    if not isinstance(probe, str):
        raise CensusError(
            f"census entry has no string `key` ({entry.get('key')!r})")
    if probe not in _registered_keys():
        return None
    ...
    acceptable = census.get("acceptable_failures")
    if not _is_x(acceptable) or acceptable != MIN_ACCEPTABLE_FAILURES:
        return None
    ...
    age = max(0.0, (now - anchor).total_seconds())
    if age >= require_horizon(stale_after_seconds):
        return None
...
def _promotion_lines(rows: list[dict]) -> list[str]:
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{row['age_seconds'] / SECONDS_PER_DAY:>7.1f}d"
...
def render_promotion_report(report: dict) -> str:
    ...
    did not run.
    """
    horizon = report["stale_after_seconds"] / SECONDS_PER_DAY
    lines = [
        ...
        f"{report['ci_eligible']} {CI_ELIGIBLE}, "
        f"{report['manual_only']} {MANUAL_ONLY}; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.promotion_candidates:
            document = load(path)
            validate_census(document, f"census {path}")
            report = promotion_report(document, **summary_arguments)
            if args.json_output:
                print(json.dumps(report, indent=2, sort_keys=True))
            else:
                sys.stdout.write(render_promotion_report(report))
            return 0
        document = load(path)
