#!/usr/bin/env python3
"""The claim NAMESPACE and the claim FILE: storage for the per-probe claim.

The filesystem leaf of `tools/probe_claim.py`'s three owners (#2148).
It knows where a claim lives, how one is encoded, how one is read back,
how one is written durably, and how the read-decide-write of a claim
file is serialized. It decides NOTHING about ownership: whether a claim
is live, expired, renewable or takeable is `probe_claim_lease`'s, and
this module neither imports that owner nor raises its `ClaimDenied`.
...
the claim. `serialized` holds it for one claim-file operation, which is
what makes TAKEOVER single-successor: reclaiming a lapsed claim is
unavoidably a read-then-write, and two reclaimers doing that
unserialized is the classic race where the second one's write lands on
top of the first one's fresh claim. Held for microseconds per
...
born already expired and any lease could lapse a second early. Every
lease decision made in the lease owner is a comparison against a stamp
parsed here, so nothing may depend on that rounding being generous.
Second-precision stamps are still ACCEPTED on read, because a claim
file is transient untracked scratch state a previous build may have
written to the second.

Malformed, incomplete and self-contradictory claims
---------------------------------------------------
`read_payload` returns `(payload_or_None, mtime_or_None)`, and the
`None` payload beside a real mtime is the MALFORMED case: empty,
truncated, unparseable, missing a field, ill-typed, or naming another
probe. This module reports it and its FILESYSTEM AGE; the lease owner
decides how long that age keeps the probe occupied. That covers a crash
between the exclusive create and the payload write without opening a
window in which a competing acquisition succeeds immediately.
