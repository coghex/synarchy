...
location's chunk loads, end to end:
  ...
  5b. Pending container shells (#2505): a location authoring a
     ...
     load. Its three fixtures (a crate item, a loot profile, and a DENSE
     location pairing them) are the probe's own: no SHIPPED location
     authors a container entry yet, because PLC-10 owns the wooden crate
     and the ruin_small entry that will carry it.
...
EVERY file this invocation creates -- its eight fixture YAMLs, its
...
def main() -> int:
    ...
    ap.add_argument("--keep-artifacts", action="store_true",
                    help="keep this run's artifact directory (its eight "
...
def run(args, art: RunArtifacts, token: str) -> int:
    """The ten-process sequence, and nothing else.

    Nine `boot_isolated` call sites; the loot-stability one runs twice
    (same order, then reversed), so a passing run launches ten engine
    ...
    #      read from. ----
    (crate_item_yaml, crate_profile_yaml,
     crate_location_yaml) = container.write_container_fixtures(art)
    proc = boot_isolated(args.port, art)
    try:
        load_registries(args.port)
