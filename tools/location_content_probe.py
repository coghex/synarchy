...
#: separately below because they behave completely differently: the loot
#: rolls are incidental and touch nothing, while the significant item
#: carries durable provenance and gates the ruin's cleared state.
...
def run(args, art: RunArtifacts, token: str) -> int:
    ...
    try:
        ...
        if not ruins:
            ...
            # Each ruin (#91, #921, #916, #917): 2 loot-table ground
            # items, ONE guaranteed significant item, and its persisted
            # uniform 0..3 nomad roll; no buildings.
            ...
            # #921/#917: the ruin's INCIDENTAL half still guarantees
            # nothing specific. `radio` and `canteen_steel_2l`
            # (spawn-only starting equipment) were the two entries #921
            # removed, and they are absent from ruin_common too — so no
            # ruin content on this page may be either. #917 added a
            # guaranteed item deliberately distinct from `radio` (D-6 in
            # docs/expedition_gameplay_loop.md reserves it for unit
            # communication), so this assertion is unchanged by it and
            # still fails if either is reinstated as a fixed entry or
            # quietly added to the loot table.
            ...
            # ---- #917: guaranteed significant contents and the
            #      COMPOUND clearance predicate. Everything here reads
            #      the engine's own reported fields
            #      (world.listPlacedLocations' `significant` array and
            #      its three clearance flags) rather than recomputing
            #      the predicate probe-side, so a divergence between
            #      what the engine decided and what this asserts is
            #      impossible to hide.
            sig1 = significant_state(args.port, "wa")
            print(f"  significant obligations: {sig1}")

            # Every placed ruin owes EXACTLY one, bound to a real
            # physical item id, and none is taken yet. The bound id is
            # the proof the content spawn registered provenance rather
            # than merely dropping an item on the floor.
            sig_errors = []
            for ruin in ruins:
                iid = int(ruin["instance_id"])
            ...
            if not sig_errors:
                ...
                failures.append(
                    f"significant obligation mismatch: {sig_errors}")

            # One physical identity per obligation, across the page.
            # Two obligations naming one item would let a single pickup
            # clear a location whose own item was never taken — the
            # session-wide rule World.Save.Integrity enforces at the
            # save boundary, checked here on the live session.
            ...
            if len(bound_ids) == len(set(bound_ids)):
                ...
                failures.append(
                    f"one physical item is owed by several obligations: "
                    f"{bound_ids}")

            # A ruin with a live nomad roster is NOT cleared however its
            # item is handled, and a zero-roll ruin is not cleared
            # either while its item is untaken — requirement 5's
            # conjunction, read straight off the engine.
            clr1 = clearance_state(args.port, "wa")
            unsatisfied = [iid for iid, (_, authors, satisfied, _)
                           in clr1.items() if authors and satisfied]
            if not unsatisfied:
                print("PASS: no ruin's clearance predicate is satisfied while "
                      "its guaranteed item is still on the floor — including "
                      "any zero-nomad roll, whose encounter half is already "
                      "complete")
            else:
                failures.append(
            ...
            # (Requirement 4 — that incidental salvage participates in
            # nothing — is proved at two cheaper tiers instead: the pure
            # "Location significant contents" spec and the IO-level
            # "compound clearance with significant contents" one both
            # pick up an unowned item through the real boundary and
            # assert nothing moves. Repeating it HERE would need a
            # player-faction looter standing in a ruin, which would
            # discover it and invalidate the sight-based discovery
            # checks this same phase runs below.)

            # A NON-PLAYER faction's pickup latches the same durable
            # state (requirement 3): the ruin was looted whoever did it.
            # Deliberately a hostile unit, and deliberately a ruin with
            # a LIVE roster where one exists, so the latch is observed
            # without the location clearing — the two halves stay
            # independent.
            occupied = next(
                (r for r in ruins
            ...
            if target is None:
                ...
                if gid is None:
                    failures.append(
                        f"ruin {tiid}'s guaranteed item is not on the ground: "
                        f"phys={phys}")
                else:
                    thief = spawn_unit(args.port, "nomad_primitive",
                                       int(target["gx"]), int(target["gy"]),
                                       "hostile", "wa")
                    took = send(
                        args.port,
                    ...
                    others_untouched = all(
                        sig2.get(i) == sig1.get(i)
                        for i in sig1 if i != tiid)
                    if took == "yes" and latched and others_untouched:
                        print(f"PASS: a hostile unit's pickup latched ruin "
                              f"{tiid}'s guaranteed item as taken, and no "
                              f"other ruin's obligations moved")
                    else:
                        failures.append(
                            f"non-player pickup did not latch as expected: "
                            f"took={took!r} sig={sig2}")
                    # …and with a live roster the location is STILL not
                    # cleared: the encounter half is outstanding.
                    if occupied is not None:
                        if not clr2.get(tiid, (None, False, False, False))[2]:
                            print(f"PASS: ruin {tiid} stays uncleared with its "
                                  f"item taken but its roster alive — both "
                                  f"halves are required")
                        else:
                            failures.append(
                                f"ruin {tiid} cleared on the item alone while "
                                f"its roster is alive: {clr2.get(tiid)}")
                    # A second pickup attempt cannot reset the latch:
                    # the item is now in an inventory, so there is no
                    # ground id to take, and the durable state is
                    # unchanged either way.
                    if significant_state(args.port, "wa") != sig2:
                        failures.append(
                            "the taken latch moved after the pickup settled")
                    else:
                        print("PASS: the taken latch is stable once set")

            # The pickup above moved one ground item into a unit, so the
            # spawn census phase 2 compares against has to be re-taken
            # here. Phase 2's claim is that a RELOAD respawns nothing —
            # a delta across the reload — not that the original numbers
            # are any particular value, and those were already asserted
