#!/usr/bin/env python3
"""The SS5 inventory-row contract (issue #876, EngineEnv capability epic
#537 requirement 8; extracted from tools/engine_env_capability_audit.py
by issue #2064).

Guards docs/engineenv_capability_inventory.md SS5 against silent drift,
the same way tools/persistence_inventory_audit.py guards the persistence
...
(`extract_record_fields`, imported through
tools/engine_env_capability_common.py rather than reimplemented)
against the SAME anchor tuple (EngineEnv / src/Engine/Core/State.hs /
the `data EngineEnv = EngineEnv` pattern) -- so this audit and the
persistence-inventory audit can never independently drift onto two
different notions of "the live EngineEnv field set."
...
Two entry points, both this owner's (#2064 requirement 14):

  `audit(live_fields, inventory_text)`
      the core, taking the ALREADY-EXTRACTED ordered field list. The
      aggregate derives that list exactly once per run and hands the
      same collection to every owner, so nothing re-reads or re-parses
      `EngineEnv` per check.
  `audit_source(engine_env_source, inventory_text)`
      the source-taking sibling: it extracts first, and keeps BOTH
      extraction guards this contract has always carried -- a
      `ValueError` from the record parser and an empty extraction each
      report as an `EngineEnv:`-prefixed violation rather than raising.
      Neither is reachable from the aggregate run, whose own extraction
      is unguarded and raises first, but they are this owner's contract
      for an injected or synthetic record -- which is how every
      self-test case feeds it -- so they stay here rather than being
...
from engine_env_capability_common import (  # type: ignore
    BACKTICK_RE, ENGINE_ENV_FILE, ENGINE_ENV_PATTERN, INVENTORY_PATH,
    SEPARATOR_ROW_RE, _is_placeholder, extract_record_fields,
)
...
def parse_inventory(text: str) -> tuple[list[ParsedRow], list[str]]:
    ...
    for line_no, line in enumerate(text.splitlines(), start=1):
        ...
        if header_idx is None:
            ...
            continue
        if all(SEPARATOR_ROW_RE.fullmatch(c) for c in cells_raw if c):
...
def audit(live_fields: list[str], inventory_text: str) -> list[str]:
    """Pure audit core. Returns a list of human-readable violations.

    `live_fields` is the ordered field list `extract_record_fields`
    returns. The aggregate derives it ONCE per run (#2064 requirement
    14) and hands the same collection to every owner; `audit_source`
    below is the sibling that takes the record TEXT instead.
    """
    violations: list[str] = []
...
def audit_source(engine_env_source: str, inventory_text: str) -> list[str]:
    """`audit` against a record's SOURCE TEXT rather than its extracted
    field list.

    The two extraction guards live here and on `audit` because they are
    this contract's answer for a record it cannot read: a parser
    `ValueError` and an empty extraction are reported as violations,
    not raised. The aggregate never reaches either -- it extracts once,
    unguarded, before any owner runs -- but an injected or synthetic
    record can, which is exactly what the self-test feeds.
    """
    try:
    ...
    return audit(live_fields, inventory_text)
