...
def test_audit_against_the_real_repo():
    ...
    expect(len(live_fields) == 87,
           f"expected 87 live EngineEnv fields (issue #876's own count of 81, "
           f"plus #907's `windowPosRef`, #957's `tutorialRegistryRef`, "
           f"#913's `playerIntentGenRef`, #1693's "
           f"`framebufferMinimizeGenRef`, #1730's `enginePauseGenRef` and "
           f"#1712's `structureWallCatalogRef`), "
