...
def test_audit_against_the_real_repo():
    ...
    expect(len(live_fields) == 85,
           f"expected 85 live EngineEnv fields (issue #876's own count of 81, "
           f"plus #907's `windowPosRef`, #957's `tutorialRegistryRef`, "
           f"#913's `playerIntentGenRef` and #1693's "
           f"`framebufferMinimizeGenRef`), got {len(live_fields)}")
