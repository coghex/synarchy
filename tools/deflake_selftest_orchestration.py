...
def test_a_lost_selection_to_claim_race_does_nothing_at_all() -> None:
    ...
    try:
        ...
        def denied(probe, **kw):
            raise probe_claim_lease.ClaimDenied(probe, owner="another agent",
                                          age_seconds=12.0)
...
def test_the_claimed_set_keeps_a_held_probe_out_of_selection() -> None:
    ...
    try:
        ...
        claim = probe_claim_lease.acquire(PROBE, root=scratch.claims,
                                    lease_seconds=600.0)
        try:
            held = deflake.claimed_probe_keys(
                registry={PROBE: "a", OTHER: "b"}, root=scratch.claims,
                now=probe_claim_storage.utc_now())
            expect(held == {PROBE},
        ...
        expired = probe_claim_lease.acquire(OTHER, root=scratch.claims,
                                      lease_seconds=0.01)
        time.sleep(0.05)
        held = deflake.claimed_probe_keys(
            registry={PROBE: "a", OTHER: "b"}, root=scratch.claims,
