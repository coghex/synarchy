#!/usr/bin/env python3
"""Offscreen probe for the "Transfer" context-menu entry (#1014, epic
#1013 phase B1).
...
Verifies, in order:
  ...
  3. activating "Transfer" creates a session
     (`scripts.transfer_session.get()`) naming the exact source/
     receiver identities;
...
Determinism (#1014 review): the technomule is spawned several tiles
away from the storage building's own tile, because
`scripts/init_context_menu.lua`'s `tryBuildingMenu` is tried before
`tryUnitMenu` -- a mule standing on the building's footprint would open
...
# Three DISJOINT interleaved candidate lists (building / mule / acolyte)
# so no two of the three fixtures can ever land on the same tile, and a
...
CANDIDATES_BUILDING = _ALL_CANDIDATES[0::3]
CANDIDATES_MULE = _ALL_CANDIDATES[1::3]
CANDIDATES_ACOLYTE = _ALL_CANDIDATES[2::3]
...
def main() -> int:
    ...
    anchor_building = find_dry_anchor(port, CANDIDATES_BUILDING)
    anchor_mule = find_dry_anchor(port, CANDIDATES_MULE)
    anchor_acolyte = find_dry_anchor(port, CANDIDATES_ACOLYTE)
    if anchor_building is None or anchor_mule is None or anchor_acolyte is None:
        sample = CANDIDATES_BUILDING[0]
        print(f"  (debug) sample tile_surface{sample}: {tile_surface(port, *sample)!r}")
    if not check("found a dry site for the storage building", anchor_building is not None):
        quit_engine(port, proc)
        return 1
    if not check("found a dry site for the technomule", anchor_mule is not None):
        quit_engine(port, proc)
        return 1
    if not check("found a dry site for the acolyte", anchor_acolyte is not None):
        quit_engine(port, proc)
        return 1
    bax, bay, _ = anchor_building
    max_, may_, _ = anchor_mule
    aax, aay, _ = anchor_acolyte

    # -- Spawn source unit, receivers.
    ...
    eligible = send_json(port, f"return unit.transferReceiverInfo('building', {bid})")
    check("building reports transfer-eligible via the new engine query",
    ...
    if check("building menu: 'Transfer' is visible", bool(transfer_row_b)):
        ...
        if ok_b:
            check("session sourceUid == the selected acolyte",
                  session_b.get("sourceUid") == uid,
                  f"got {session_b.get('sourceUid')!r}")
            check("session receiverKind == 'building'",
                  session_b.get("receiverKind") == "building",
                  f"got {session_b.get('receiverKind')!r}")
            check("session receiverId == the storage building",
                  session_b.get("receiverId") == bid,
                  f"got {session_b.get('receiverId')!r}")
            contract_b = session_b.get("contract") or {}
            check("session contract.operation == 'unit_to_building_storage'",
                  contract_b.get("operation") == "unit_to_building_storage",
                  f"got {contract_b!r}")
    clear_session(port)
    ...
    # ------------------------------------------------------------------
    print("== unit receiver (technomule) ==")
    resolved_m = center_on_tile(port, max_, may_, cx0, cy0)
    check("camera resolves the technomule's tile area", resolved_m == (max_, may_),
          f"got {resolved_m!r}")
    time.sleep(0.3)
    pixel = locate_unit_pixel(port, mule_uid, fb_w, fb_h)
    if not check("located the technomule's own screen pixel via hitTestInRect",
                 pixel is not None):
        quit_engine(port, proc)
        return 1
    px, py = pixel

    right_click_and_check_routing(port, px, py, "context_menu_unit", "unit")
    ...
    if check("unit menu: 'Transfer' is visible on the technomule", bool(transfer_row_u)):
        ...
        if ok_u:
            check("session sourceUid == the selected acolyte",
                  session_u.get("sourceUid") == uid,
                  f"got {session_u.get('sourceUid')!r}")
            check("session receiverKind == 'unit'",
                  session_u.get("receiverKind") == "unit",
                  f"got {session_u.get('receiverKind')!r}")
            check("session receiverId == the technomule",
                  session_u.get("receiverId") == mule_uid,
                  f"got {session_u.get('receiverId')!r}")
            contract_u = session_u.get("contract") or {}
            check("session contract.operation == 'unit_to_unit_inventory'",
                  contract_u.get("operation") == "unit_to_unit_inventory",
                  f"got {contract_u!r}")
