...
def test_namespace() -> None:
    ...
    with registry(), scratch_repo() as (main_wt, other_wt, _census):
        ...
        expect(here == there,
               ...
               "main checkout")
        expect(here.is_absolute()
               and here.name == claim_storage.CLAIM_DIR_NAME,
...
def test_exclusive_acquisition() -> None:
    ...
    with registry(), claim_root() as root:
        ...
        expect(first.token
               and claim_storage.claim_path("alpha", root).exists(),
...
def test_expiry_is_one_way() -> None:
    ...
    with registry(), claim_root() as root:
        ...
        expect(claim_lease.read_claim("beta", root=root)["expires_at"]
               > before,
...
def test_release_on_every_managed_exit() -> None:
    ...
    with registry(), claim_root() as root:
        ...
        for kind in (Boom, KeyboardInterrupt, SystemExit):
            try:
                with claim_lease.acquire("alpha", root=root,
                                         lease_seconds=600):
                    raise kind("stopped")
