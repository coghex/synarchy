...
This is the COMMAND, and nothing else:

    python3 tools/probe_claim.py --probe role --runs 10 --result /tmp/r.json
    python3 tools/probe_claim.py --status

Since #2148 the implementation lives in three owners beside it, each
importable on its own and each carrying the invariants it enforces:

  `probe_claim_storage`        the namespace under the repository-common
                               git directory, the claim-file codec, the
                               durable writes, and the sidecar `flock`
                               that serializes one read-decide-write
  `probe_claim_lease`          probe-key and lease validation, the
                               token, acquisition, renewal, takeover,
                               token-safe release, `Renewer`, and the
                               status query behind `--status`
  `probe_claim_orchestration`  the claimed measurement: result-path
                               validation, the acquisition audit, the
                               pre-run reassertion, retention of a
                               completed measurement, serialized
                               ingestion, and the `EXIT_*` table

Dependencies run one way — storage is the filesystem leaf, lease builds
on storage and the probe registry, orchestration builds on both plus
`probe_flake` and `probe_census`, and this file builds on the three. A
facility has exactly ONE owner and this file re-exports none of them: a
tool or test that reaches for `acquire`, `Claim`, `read_claim`,
`retain_measurement`, `LEASE_SECONDS` or an exit constant imports the
owner, so assigning to it changes the state the implementation reads.
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--lease-seconds", type=float,
                    default=lease.LEASE_SECONDS,
