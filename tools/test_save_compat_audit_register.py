...
def test_generate_session_rolls_back_when_the_summary_destination_is_unusable() -> None:
    print("issue #2273: --summary naming a directory that does not exist "
          "must be an ordinary dump FAILURE, not an exception -- "
          "cmd_generate rolls the fixture back on a returned failure, so an "
          "OSError escaping dump_canonical_summary's staging would leave a "
          "newly generated, unregistered fixture on disk. The pre-#2273 "
          "path could not do that: the helper itself failed on an "
          "unwritable output and exited non-zero")
    with tempfile.TemporaryDirectory(dir=common.REPO_ROOT) as d:
        tmp = Path(d)
        fixture_path = tmp / "gen.bin"
        # A parent that genuinely is not there. Nothing creates it: the
        # point is that the destination is unusable before the helper is
        # ever reached.
        summary_path = tmp / "no-such-directory" / "gen.expected.json"
        old_gen = generate.generate_current_format_session
        called = []
        generate.generate_current_format_session = (
            lambda **kw: (called.append("gen"),
        ...
        try:
            rc = generate.cmd_generate(_Args(
                ...
                summary=str(summary_path.relative_to(common.REPO_ROOT)),
                force=True))
            expect(rc == 1, f"expected a clean failure exit, got {rc}")
            expect(called == ["gen"],
                   f"expected generation to have been reached and faked "
                   f"(otherwise this proves nothing about rollback), got "
                   f"{called}")
            expect(not fixture_path.exists(),
                   "expected the newly generated fixture to be rolled back "
                   "-- it did not exist before this invocation, so rollback "
                   "means removing it, and leaving it behind is the "
                   "unregistered-fixture litter the transaction exists to "
                   "prevent")
        finally:
            generate.generate_current_format_session = old_gen


def test_dump_canonical_summary_reports_an_unusable_destination_as_failure() -> None:
    print("issue #2273, the same defect at its own seam: the codec bridge "
          "answers (False, diagnostic) for a destination it cannot write, "
          "naming both the fixture and the path, rather than raising")
    with tempfile.TemporaryDirectory(dir=common.REPO_ROOT) as d:
        destination = Path(d) / "no-such-directory" / "summary.json"
        fixture = common.REPO_ROOT / (
            "test-headless/data/save-compat/u1-generated-world-identity.bin")
        ok, tail = codec.dump_canonical_summary(fixture, destination)
        expect(not ok, "expected an unwritable destination to fail")
        expect(str(destination) in tail and str(fixture) in tail,
               f"expected the diagnostic to name both the destination and "
               f"the fixture, got {tail!r}")
...
TESTS = [
    ...
    test_generate_session_rolls_back_when_the_summary_destination_is_unusable,
    test_dump_canonical_summary_reports_an_unusable_destination_as_failure,
]
