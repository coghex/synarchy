...
already committed, is append-only, and is neither retried nor rolled
back, so both facts are reported and `handoff_document` stays null. The
outcome vocabulary does not grow: an eleventh step that invented a new
one would make every caller's branch table wrong.
...
import probe_claim_lease  # noqa: E402
import probe_claim_orchestration  # noqa: E402
import probe_claim_storage  # noqa: E402
...
def claimed_probe_keys(*, registry, root, now) -> set:
    ...
    authority on ownership is `probe_claim_lease.acquire`, which is why losing
    ...
    the same safe direction `probe_claim_lease.acquire` takes with one.
    """
    base = Path(root)
    claimed = set()
    if not base.is_dir():
    ...
    for key in sorted(registry):
        try:
            document = probe_claim_lease.read_claim(key, root=base)
        except probe_claim_storage.ClaimError:
            claimed.add(key)
            continue
        if document is None:
            continue
        expires = probe_claim_storage.parse_stamp(document.get("expires_at"))
...
def _require_commit(value, what: str) -> str:
    ...
    `probe_flake._commit_sha` and `probe_claim_storage.commit_sha` both answer
...
def measure_next_probe(*, repo_root=None, census_path=None, claim_root=None,
                       ...
                       lease_seconds: float = probe_claim_lease.LEASE_SECONDS,
                       now=None,
    ...
    take_claim = (acquire_claim if acquire_claim is not None
                  else probe_claim_lease.acquire)
    ...
    read_head = (head_commit if head_commit is not None
                 else (lambda: probe_claim_storage.commit_sha(repo_root)))
    ...
    try:
        claim_base = (
            Path(claim_root) if claim_root is not None
            else probe_claim_storage.repository_claim_root(repo_root))
...
def _release(claim) -> str | None:
    ...
    Token-checked by `probe_claim_lease.Claim.release`, so a claim that has
...
def _measure_claimed(*, probe, claim, selection, target, repo_root, namespace,
    ...
    except probe_claim_lease.ClaimLost as error:
    ...
    try:
        with hold:
            ...
            say(f"measuring {probe!r}: {runs} runs at {rts_caps} RTS "
                f"capabilities")
            with probe_claim_lease.Renewer(claim):
    ...
    retained, retain_problem = probe_claim_orchestration.retain_measurement(
