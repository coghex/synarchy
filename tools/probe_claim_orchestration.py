#!/usr/bin/env python3
"""The claimed measurement: claim, audit, measure, retain, ingest (#2148).

The top owner of `tools/probe_claim.py`'s three. `run_claimed_measurement`
is where the claim is worth having, and this module holds the ordering
that makes it worth having. It builds on `probe_claim_lease` for
ownership and `probe_claim_storage` for the claim file, and adds the
two collaborators neither of them may know about: `probe_flake`, which
measures, and `probe_census`, which records.

The order, and every early exit
-------------------------------
...
The lease floor
---------------
`probe_claim_lease.acquire` accepts any positive lease, so a test can
drive expiry deliberately. This boundary REFUSES one shorter than
...
mid-measurement. It is refused rather than silently raised, because a
caller asking for thirty seconds has misunderstood what this lease is
for. The ownership check before ingestion is the backstop for
everything a floor cannot foresee, such as a machine suspended for an
hour.

The exit codes
--------------
The whole `EXIT_*` table lives here, and `tools/probe_claim.py` imports
it. `run_claimed_measurement` and `Outcome` decide three of them and
the command decides the other four, so the command is the wrong home:
putting any of them there would make this module import upward and
break the one-way dependency direction. The command's own docstring
carries the operator-facing PROSE for each; the numbers have exactly
one definition, and it is this one.

Every collaborator of the two lower owners' is reached MODULE-QUALIFIED
at call time (`lease.acquire(...)`, `storage.commit_sha(...)`), never
through a `from ... import`, so a caller that substitutes one is
changing the state this module actually reads.
...
def run_claimed_measurement(probe: str, runs: int, *,
    ...
    * a measurement that finishes without the claim still being ours
    ...
    if runs < 1:
        raise storage.ClaimError(f"--runs must be a positive count, got {runs}")
