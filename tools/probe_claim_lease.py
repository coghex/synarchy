#!/usr/bin/env python3
"""Ownership: who holds a probe's claim, for how long, and what may write it.

The ownership owner of `tools/probe_claim.py`'s three (#2148). It
consumes `probe_claim_storage` for the namespace, the claim-file codec
and the sidecar lock, and `probe_runner_registry` for the canonical
probe keys; it knows nothing of the census, the measurement, or the
command. Every DECISION about a claim is made here: whether it is live,
whether a malformed file has aged past its lease, whether a renewal is
still allowed, and which of several concurrent reclaimers wins.

Identity
--------
Claims are keyed by the canonical `probe_runner_registry.PROBES` key and
nothing else, so the two spellings of a probe cannot claim it twice.
Validating against the registry also means no claim path is ever built
from an unvetted string.
...
`acquire` accepts any positive lease, so the self-test can drive expiry
deliberately. The FLOOR that makes a lease usable in practice belongs to
the orchestration boundary, which is the only caller that knows a
measurement is about to run.
...
A claim file storage reports as MALFORMED — empty, truncated,
unparseable or incomplete — is treated as OCCUPIED until its own
filesystem age reaches the lease, then becomes reclaimable. Storage
supplies that age; this module compares it against the lease in force
for the acquisition and denies or takes over accordingly. That covers a
crash between the exclusive create and the payload write without
opening a window in which a competing acquisition succeeds immediately.

`commit_while_held` is the one seam anything that PUBLISHES must use.
Checking ownership and then acting on the answer are two steps, and
between them the lease can lapse and another agent can take the probe;
so the check, the renewal and the caller's commit all happen inside ONE
hold of the sidecar lock.

`status_rows` is the read-only status QUERY, so the command can report
every claim in a namespace without touching storage's codec itself.
...
def require_lease(value, what: str = "a claim lease") -> float:
    ...
    `storage.MAX_LEASE_SECONDS` is where the census's own cap sits, so a lease
...
def status_rows(root: Path) -> list[dict]:
    """Every claim in `root`, as one read-only row per occupied probe.

    The status QUERY, so that the command reading it needs nothing of
    storage's file codec: the decode, the malformed-versus-live
    distinction and the expiry comparison are all ownership questions
    and all answered here. The command turns these rows into text or
    JSON and decides nothing.

    A probe with no claim file contributes no row at all; one whose
    claim cannot even be examined contributes an `unusable` row rather
    than raising, because `--status` is the operator's way of finding
    out that something is wrong with the namespace.
    """
    rows = []
    if not root.is_dir():
        return rows
