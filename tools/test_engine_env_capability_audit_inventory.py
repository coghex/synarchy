...
capability-inventory gap using synthetic fixtures, never by editing the
real EngineEnv or the real inventory doc. Every case here feeds
`audit`/`parse_inventory` the shared synthetic Haskell record and a
synthetic inventory document from
`test_engine_env_capability_audit_support`: missing, duplicate and
stale rows; capability headings; the lifecycle and thread-role grammar
(each review round's literal shape kept as its own case); the
reader/writer decisions; the synchronization, lifecycle, notes and
grounding-evidence cells; and section-5 bounds. The one real-repository
case, `test_audit_against_the_real_repo`, says so and runs last.
...
from test_engine_env_capability_audit_support import (  # noqa: E402
    ...
    SYNTHETIC_ENGINE_ENV, SYNTHETIC_INVENTORY_COMPLETE, expect,
...
def test_malformed_capability_heading_resets_scope():
    ...
    doc = (
        ...
        "## 5. Field inventory\n\n"
        f"### core-init\n\n{INVENTORY_HEADER}{FIELD_ONE_ROW}\n"
        "###\n\n"
...
def test_audit_against_the_real_repo():
    ...
    expect(len(live_fields) == 91,
           f"expected 91 live EngineEnv fields (issue #876's own count of 81, "
           ...
           f"#2020's `maxImageDimensionRef`, #2297's `treatRNGRef` and "
           f"#2499's `lootProfileRegistryRef`, "
           f"minus #2285's write-only `popupQueueRef`), "
